...
import { JWK, IKeyStore, IJWKKey } from 'node-jose';
...
function isDefinedAndMatches(metadata: {[key: string]: any}) {
...
function isDefinedAndStartsWithHS(metadata: {[key: string]: any}) {
  return (prop: string): boolean => {
    const value = metadata[prop];
...
export default function getClient(provider) {
  ...
  async function sectorValidate(client: Client) {
  ...
  async function buildKeyStore(client: Client) {
    ...
    if (client.jwks && client.jwks.keys) {
      client.jwks.keys.forEach((key: IJWKKey) => {
        if (handled(key.kty)) {
  ...
  function add(metadata, dynamic?: boolean, ttl = instance(provider).configuration('clientCacheDuration') * 1000) {
