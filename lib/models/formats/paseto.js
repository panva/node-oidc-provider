...
module.exports = (provider, { opaque }) => {
  async function getResourceServerConfig(token) {
    ...
    if (purpose === 'local' && version === 1) {
      ...
      if (version === 1) {
        key = keystore.get({ kty: 'RSA', kid });
        if (key && key.length !== 2048) {
          throw new Error('v1.public paseto keys must have modulus length of 2048 bits');
        }
      } else if (version === 2) {
        key = keystore.get({ kty: 'OKP', crv: 'Ed25519', kid });
      }
      if (!key) {
        throw new Error('resolved Resource Server paseto configuration has no corresponding key in the provider\'s keystore');
      }
      ({ kid, keyObject: key } = key);
    } else {
