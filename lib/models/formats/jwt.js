...
module.exports = (provider, { opaque }) => {
  async function getSigningAlgAndKey(ctx, token) {
    const { client } = token;
    const { keystore, configuration } = instance(provider);
    const { formats: { tokenSigningAlg }, clientDefaults } = configuration();

    const alg = (await tokenSigningAlg(ctx, token, client))
      || clientDefaults.id_token_signed_response_alg;
  ...
  return {
    ...
    async getValueAndPayload() {
      ...
      if (this[LOOKUP_VALUE]) {
        ...
        const { key, alg } = await getSigningAlgAndKey(ctx, this);
