...
export default function getBaseToken(provider) {
  ...
  return class BaseToken {
     ...
     */
    get expiration() {
      return this.expiresIn || (this.constructor as typeof BaseToken).expiresIn;
    }
     ...
     *
     */
    async getValueAndPayload<T>(): Promise<[string, {
      grantId?: string,
      header: any,
      payload: any,
      signature: string
    }]> {
      const jwt = await JWT.sign(pick(this, (this.constructor as typeof BaseToken).IN_PAYLOAD), undefined, 'none', {
     ...
     *
     */
    static getTokenId(token: string) {
     ...
     *
     */
    static async verify(token: string, stored, options: { ignoreExpiration: boolean }) {
