...
module.exports = function getIdToken(provider) {
  ...
  return class IdToken {
    ...
    async sign(client, options = {}) {
      options.use = 'use' in options ? options.use : 'idtoken';
      options.expiresAt = 'expiresAt' in options ? options.expiresAt : null;
      options.noExp = 'noExp' in options ? options.noExp : null;

      const expiresIn = (() => {
        if (options.expiresAt) return options.expiresAt - epochTime();
        return undefined;
      })();

      const alg = options.use === 'userinfo' ? client.userinfoSignedResponseAlg : client.idTokenSignedResponseAlg;
      ...
      const signed = await (() => {
        ...
        return JWT.sign(payload, key, alg, {
          ...
          expiresIn: options.noExp ? undefined : (expiresIn || this.constructor.expiresIn),
          issuer: provider.issuer,
          subject: payload.sub,
        });
      })();

      const encryption = options.use === 'userinfo' ? {
