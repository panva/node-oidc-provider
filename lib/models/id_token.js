  ...
  return class IdToken {
    ...
    static expiresIn(...args) {
      ...
      if (typeof ttl === 'number') {
        return positiveInteger(ttl, `ttl.${this.name}`);
      }

      return positiveInteger(ttl(...args), `ttl.${this.name}`);
    }
    ...
    async issue({ use, expiresAt = null } = {}) {
      ...
      const signed = await (async () => {
        if (typeof alg !== 'string') {
          throw new Error(`${use} signing algorithm must be a string`, { cause: client });
        }
        let jwk;
        let key;
      ...
      if (encryption.alg === 'dir') {
        ...
        if (jwk) encryptionKey = client.symmetricKeyStore.getKeyObject(jwk, true);
        ...
        if (jwk) encryptionKey = client.asymmetricKeyStore.getKeyObject(jwk, true);
      }
