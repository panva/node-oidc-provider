...
const getMask = require('../helpers/claims_mask');
const JWT = require('../helpers/jwt');

const hashed = ['at_hash', 'c_hash', 'rt_hash'];

module.exports = function getIdToken(provider) {
  const ClaimsMask = getMask(provider.configuration);
  ...
  return class IdToken {
    ...
    payload() {
      const mask = new ClaimsMask(this.available, this.sector);
