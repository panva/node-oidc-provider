...
function getIdToken(provider) {
    ...
    return class IdToken {
        constructor(available, sector) {
            assert.equal(typeof available, 'object', 'expected claims to be an object, are you sure claims() method resolves with or returns one?');
            this.extra = {};
        ...
        async sign(client, { use = 'idtoken', audiences = null, expiresAt = null, noExp = null, } = {}) {
            const expiresIn = (() => {
                if (expiresAt != null)
                    return expiresAt - epochTime();
                return undefined;
            })();
