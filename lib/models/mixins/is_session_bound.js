...
module.exports = provider => superclass => class extends superclass {
  ...
  static async find(...args) {
    ...
    try {
      ...
      assert.deepEqual(token.accountId, session.accountId(), 'token and session principal are now different');

      // session is still the same grantId
      assert.deepEqual(token.grantId, session.grantIdFor(token.clientId), 'client\'s token and session grantId are now different');
