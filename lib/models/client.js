...
module.exports = function getClient(provider) {
  ...
  function buildKeyStore(client) {
    ...
    keystore.setReferences(client, provider);

    Object.defineProperty(client, 'keystore', { value: keystore });
    ...
    if (client.clientSecret) {
      ...
      for (const alg of [...algs].filter(Boolean)) { // eslint-disable-line no-restricted-syntax
        let key;
        if (alg === 'HS') {
          key = jose.JWK.asKey({
            use: 'sig', kty: 'oct', k: base64url.encode(client.clientSecret),
          });
          ...
          key = jose.JWK.asKey({
            alg, kty: 'oct', k: base64url.encodeBuffer(derived), use: 'enc',
          });
        } else if (alg.startsWith('PBES2')) {
          key = jose.JWK.asKey({
            alg, kty: 'oct', k: base64url.encode(client.clientSecret), use: 'enc',
          });
        }
