...
function checkJWK(prop, jwk, i) {
  if (!_.isPlainObject(jwk) || typeof jwk.kty !== 'string' || !jwk.kty) {
  ...
  try {
    return jose.JWK.asKey(_.pick(jwk, KEY_ATTRIBUTES));
  } catch (err) {
...
module.exports = function getClient(provider) {
  ...
  function buildKeyStore(client) {
              ...
              if (opts && opts.alg && algs.has(opts.alg)) {
                ...
                if (opts.alg.startsWith('HS')) {
                  ...
                  const derived = crypto.createHash('sha256')
                    .update(this.client.clientSecret)
                    .digest()
                    .slice(0, len);

                  key = jose.JWK.asKey({
                    alg: opts.alg, kty: 'oct', k: base64url.encodeBuffer(derived), use: 'enc',
                  });
                  ...
                  key = jose.JWK.asKey({
                    alg: opts.alg, kty: 'oct', k: base64url.encode(this.client.clientSecret), use: 'enc',
                  });
                }
    ...
    if (client.clientSecret) {
      ...
      algs.add(client.idTokenEncryptedResponseAlg);
      if (client.idTokenEncryptedResponseAlg === 'dir') {
        algs.add(client.idTokenEncryptedResponseEnc);
      }

      algs.add(client.userinfoEncryptedResponseAlg);
      if (client.userinfoEncryptedResponseAlg === 'dir') {
        algs.add(client.userinfoEncryptedResponseEnc);
      }

      algs.add(client.introspectionEncryptedResponseAlg);
      if (client.introspectionEncryptedResponseAlg === 'dir') {
        algs.add(client.introspectionEncryptedResponseEnc);
      }

      algs.add(client.authorizationEncryptedResponseAlg);
      if (client.authorizationEncryptedResponseAlg === 'dir') {
        algs.add(client.authorizationEncryptedResponseEnc);
      }
  ...
  function addStatic(metadata) {
    if (!_.isPlainObject(metadata) || !metadata.client_id) {
    ...
    if (staticCache.has(metadata.client_id)) {
      throw new InvalidClientMetadata('client_id must be unique for statically configured clients');
    }

    staticCache.set(metadata.client_id, _.cloneDeep(metadata));
  }
