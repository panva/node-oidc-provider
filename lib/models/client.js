...
module.exports = function getClient(provider) {
  ...
  const adapter = new (provider.configuration('adapter'))('Client');
  ...
  function store(client) {
    return adapter.upsert(client.clientId, client.metadata()).then(() => client);
  }
  ...
  class Client {
    ...
    static find(id) {
      ...
      return adapter.find(id).then((properties) => {
        if (properties) {
