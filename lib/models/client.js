...
module.exports = function getClient(provider) {
  ...
  async function add(metadata, { store = false, static: statik = false } = {}) {
    const client = await schemaValidate(new Client(), metadata); // eslint-disable-line no-use-before-define, max-len

    await sectorValidate(client);
    await buildKeyStore(client);
  ...
  class Client {
    ...
    static async find(id) {
      ...
      if (!client) {
        client = await add(properties, { static: false, store: false });
