...
module.exports = function getClient(provider) {
  ...
  function sectorValidate(client) {
    ...
    return got(client.sectorIdentifierUri, provider.httpOptions()).then(({ statusCode, body }) => {
      try {
        assert.equal(
          statusCode, 200,
      ...
      throw new InvalidClientMetadata(`could not load sector_identifier_uri (${error.message})`);
    });
  }
  ...
  function buildKeyStore(forClient) {
    ...
    // TODO: failed RP fetch should not fail start of an OP (sector_identifier_uri, jwks_uri)

    Object.defineProperties(client.keystore, {
      jwksUri: {
        get() {
          return client.jwksUri;
        },
      },
      fresh: {
        value() {
      ...
      refresh: {
        value() {
          return got(this.jwksUri, provider.httpOptions()).then(({ headers, body, statusCode }) => {
            let freshUntil;
    ...
    if (client.jwksUri) promises.push(client.keystore.refresh());
