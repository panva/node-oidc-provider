...
function isDefinedAndMatches(metadata) {
    return (prop) => {
        const value = metadata[prop];
        return value !== undefined && String(value).match(/^(A|P)/) != null;
    };
}
function isDefinedAndStartsWithHS(metadata) {
    return (prop) => {
        const value = metadata[prop];
...
function stripFragment(uri) {
    return url_1.format(new url_1.URL(uri), { fragment: false });
}
...
Object.defineProperties(JWKStore.prototype, {
    ...
    fresh: {
        value() {
            if (!this.jwksUri)
                return true;
            const now = epoch_time_1.default();
    ...
    refresh: {
        async value() {
            try {
                const { headers, body, statusCode, } = await httpRequest.get(this.jwksUri, this.provider.httpOptions());
                let freshUntil;
                if (freshUntil = Date.parse(headers.expires)) {
                    freshUntil = epoch_time_1.default(freshUntil);
                }
                else if (headers['cache-control'] && headers['cache-control'].match(/max-age=(\d+)/)) {
                    const maxAge = parseInt(RegExp.$1, 10);
                    freshUntil = epoch_time_1.default() + maxAge;
                }
                else {
                    // TODO: configurable
                    freshUntil = epoch_time_1.default() + 60;
                }
                this.freshUntil = freshUntil;
                assert.equal(statusCode, 200, `unexpected jwks_uri statusCode, expected 200, got ${statusCode}`);
...
function getClient(provider) {
    ...
    async function sectorValidate(client) {
        if (client.sectorIdentifierUri === undefined)
            return client;
        try {
            const { statusCode, body, } = await httpRequest.get(client.sectorIdentifierUri, provider.httpOptions())
                .catch((err) => {
                throw new Error(`could not load sector_identifier_uri (${err.message})`);
            });
            assert.equal(statusCode, 200, `unexpected sector_identifier_uri statusCode, expected 200, got ${statusCode}`);
            const parsedBody = JSON.parse(body);
            ...
            assert(!missing, 'all registered redirect_uris must be included in the sector_identifier_uri');
            return client;
        }
        catch (err) {
    ...
    async function buildKeyStore(client) {
        ...
        _.chain(['idTokenEncryptedResponseAlg', 'userinfoEncryptedResponseAlg'])
            ...
            if (alg.startsWith('HS')) {
                promises.push(client.keystore.add({ alg, kty: 'oct', k: base64url_1.default(client.clientSecret) }));
            }
                ...
                const key = crypto.createHash('sha256')
                    ...
                    .digest()
                    .slice(0, len);
                promises.push(client.keystore.add({ alg, kty: 'oct', k: base64url_1.default(key) }));
            }
            return undefined;
        })
            .value();
    ...
    class Client {
        ...
        redirectUriAllowed(redirectUri) {
            ...
            if (this.applicationType === 'native' &&
                ...
                try {
                    const parsed = new url_1.URL(redirectUri);
