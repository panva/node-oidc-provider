...
import { isValidClientIdUrl, resolveClientByMetadataDocument } from '../helpers/client_id_metadata_document.js';
...
const CLIENT_JWK_REQUIREMENTS = new Map([
  ['EC', { required: ['x', 'y'], privateParameters: ['d'], subtype: { parameter: 'crv', values: EC_CURVES } }],
  ['OKP', { required: ['x'], privateParameters: ['d'], subtype: { parameter: 'crv', values: OKP_SUBTYPES } }],
  ['AKP', { required: ['alg', 'pub'], privateParameters: ['priv'] }],
  ['RSA', { required: ['e', 'n'], privateParameters: ['d', 'p', 'q', 'dp', 'dq', 'qi', 'oth'] }],
]);

function stripLoopbackPort(uri) {
  const parsed = URL.parse(uri);
  if (!parsed || parsed.protocol !== 'http:' || !LOOPBACKS.has(parsed.hostname)) {
    return undefined;
  }
  ...
  if (hostAndPort.startsWith('[')) {
    ...
    if (bracketEnd === -1 || hostAndPort[bracketEnd + 1] !== ':') {
      return uri;
    }
    portStart = hostStart + bracketEnd + 1;
  } else {
    const portOffset = hostAndPort.lastIndexOf(':');
    if (portOffset === -1) {
      return uri;
    }
    portStart = hostStart + portOffset;
  }

  if (!/^\d*$/u.test(authority.slice(portStart + 1))) {
    return undefined;
  }

  return `${uri.slice(0, authorityStart + portStart)}${uri.slice(authorityEnd)}`;
}
...
function checkJWK(jwk, i) {
  const prefix = `jwks.keys[${i}]`;

  try {
    const invalid = (message) => {
      throw new Error(message, { cause: jwk });
    };
    const requireString = (parameter, { keyType, optional = false } = {}) => {
      if (optional && jwk[parameter] === undefined) return;

      if (!(typeof jwk[parameter] === 'string' && jwk[parameter])) {
        const qualifier = keyType ? ` for ${keyType} keys` : optional ? ' if provided' : '';
        invalid(`${prefix}.${parameter} must be a non-empty string${qualifier}`);
      }
    };

    if (!(isPlainObject(jwk))) invalid(`${prefix} must be a plain object`);
    requireString('kty');

    const requirements = CLIENT_JWK_REQUIREMENTS.get(jwk.kty);
    if (!requirements) {
      if (jwk.kty === 'oct') invalid(`${prefix} must not contain symmetric keys`);
      return undefined;
    }

    const keyType = jwk.kty;
    if (requirements.subtype) {
      requireString(requirements.subtype.parameter, { keyType });
      if (!requirements.subtype.values.has(jwk[requirements.subtype.parameter])) return undefined;
    }

    for (const parameter of requirements.required) {
      requireString(parameter, { keyType });
    }

    for (const parameter of requirements.privateParameters) {
      if (jwk[parameter] !== undefined) {
        invalid(`${prefix}.${parameter} must not be provided for ${keyType} keys`);
      }
    }

    for (const parameter of ['alg', 'kid', 'use']) {
      requireString(parameter, { optional: true });
    }

    if (!(jwk.x5c === undefined || (Array.isArray(jwk.x5c) && jwk.x5c.every((x) => typeof x === 'string' && x)))) {
      invalid(`${prefix}.x5c must be an array of non-empty strings if provided`);
    }
  } catch (cause) {
    throw new InvalidClientMetadata('client JSON Web Key Set is invalid', { cause });
  }
...
function deriveEncryptionKey(secret, length) {
  const digest = length <= 32 ? 'sha256' : length <= 48 ? 'sha384' : length <= 64 ? 'sha512' : false;
  if (!digest) {
...
export default function getClient(provider) {
  class ClientKeyStore extends KeyStore {
    ...
    async refresh() {
      ...
      if (!this.lock) {
        this.lock = (async () => {
          const response = await fetchRequest(provider, new URL(this.jwksUri).href, {
            method: 'GET',
            headers: {
          ...
          if (status !== 200) {
            throw new Error(`unexpected jwks_uri response status code, expected 200 OK, got ${status} ${STATUS_CODES[status]}`, { cause: response });
          }
          ...
          delete this.lock;
          throw new InvalidClientMetadata('client JSON Web Key Set failed to be refreshed', { cause });
        });
      }
  ...
  class Client {
    ...
    constructor(metadata, ctx, { cimd } = {}) {
      ...
      Object.assign(this, mapKeys(schema, (_value, key) => {
    ...
    async backchannelPing(backchannelAuthenticationRequest) {
      ...
      return fetchRequest(provider, new URL(this.backchannelClientNotificationEndpoint).href, {
        method: 'POST',
        redirect: 'manual',
        headers: {
    ...
    async backchannelLogout(sub, sid) {
      ...
      return fetchRequest(provider, new URL(this.backchannelLogoutUri).href, {
        method: 'POST',
        redirect: 'manual',
        headers: {
    ...
    #redirectAllowed(value, allowedUris) {
      ...
      const withoutPort = stripLoopbackPort(value);
      if (withoutPort === undefined) {
        return false;
      }

      return allowedUris.some((allowed) => stripLoopbackPort(allowed) === withoutPort);
    }
    ...
    metadata() {
      return mapKeys(this, (_value, key) => {
    ...
    static async find(id) {
      ...
      if (
        instance(provider).features.clientIdMetadataDocument.enabled
        && isValidClientIdUrl(id)
      ) {
        return resolveClientByMetadataDocument(provider, id);
      }
