...
function validateCertificateChain(jwk) {
  try {
    const x5c = jwk[X5C];
    assert(Array.isArray(x5c) && x5c.length, 'when provided, x5c must be a non-empty array');
    const cert = x5c[0];
    const crt = jose.JWK.importKey({
      key: `-----BEGIN CERTIFICATE-----\n${cert}\n-----END CERTIFICATE-----`,
      format: 'pem',
    });
    Object.entries(crt.toJWK(false)).forEach(([key, value]) => {
      if (key === 'kid') return;
      assert.equal(jwk[key], value, `cert and key ${key} mismatch`);
    });
  } catch (err) {
    throw new InvalidClientMetadata(`invalid x5c provided (${err.message})`);
  }
}
