...
module.exports = function (provider) {
  ...
  let baseSchema = function () {
    ...
    return Joi.object().keys({
      ...
      grant_types: Joi.when('response_types', {
        is: Joi.array().items(Joi.string().regex(/code/).required()),
        then: Joi.array().min(1).items(Joi.string().valid('authorization_code')
          .required(), conf.grantTypesSupported),
        otherwise: Joi.alternatives().when('response_types', {
          is: Joi.array().items(Joi.string().regex(/token/).required()),
          then: Joi.array().min(1).items(
            Joi.string().valid('implicit').required(),
              conf.grantTypesSupported),
          }),
