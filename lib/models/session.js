...
module.exports = (provider) => class Session extends hasFormat(provider, 'Session', instance(provider).BaseModel) {
  ...
  static async findSessionByLoginHint(accountId) {
    const stored = await this.adapter.findSessionByLoginHint(accountId);
    try {
      assert(stored);
      const payload = await this.verify(stored);
      return this.instantiate(payload);
  ...
  static async get(ctx) {
    ...
    if (!session) {
      ...
      if (cookieSessionId) {
        ...
        if ((url && loginHint)) {
          const loginHintResponse = await this.findAccountByLoginHint(loginHint)
