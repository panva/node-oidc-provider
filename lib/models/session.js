...
module.exports = function getSession(provider) {
  const adapter = new (provider.configuration('adapter'))('Session');
  ...
  class Session {
    ...
    save() {
      ...
      return adapter.upsert(this.id, payload,
        provider.configuration('cookies.long.maxAge') / 1000);
    }

    destroy() {
      return adapter.destroy(this.id);
    }
    ...
    static find(id) {
      return adapter.find(id).then(data => new Session(id, data));
    }
