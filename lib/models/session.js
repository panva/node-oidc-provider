...
module.exports = (provider) => class Session extends hasFormat(provider, 'Session', instance(provider).BaseModel) {
  ...
  static async findSessionByAccountId(accountId) {
    const stored = await this.adapter.findSessionByAccountId(accountId);
    try {
      assert(stored);
      const payload = await this.verify(stored);
      return this.instantiate(payload);
  ...
  static async get(ctx) {
    ...
    if (!session) {
      if (cookieSessionId) {
        ...
        // const url = new URL(`${ctx.request.header.host}${ctx.request.url}`)
        // // login_hint = accountId
        // const loginHint = url.searchParams.get('login_hint')
        // if ((url && loginHint)) {
        //   const loginHintResponse = await this.findSessionByAccountId(loginHint)
        //   session = this.instantiate(loginHintResponse)
        // } else {
          session = this.instantiate();
        // }
      }
