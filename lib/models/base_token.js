...
function getBaseToken(provider) {
    ...
    return class BaseToken {
        constructor(payload) {
            ...
            this.jti = this.jti || base64url_1.default(uuid_1.v4());
        ...
        static async find(token = '', { ignoreExpiration = false } = {}) {
            ...
            try {
                ...
                if (stored.consumed !== undefined)
                    inst.consumed = stored.consumed;
                return inst;
            }
            catch (err) {
                if (rethrow)
        ...
        async getValueAndPayload() {
            const jwt = await JWT.sign(lodash_1.pick(this, this.constructor.IN_PAYLOAD), undefined, 'none', {
                expiresIn: this.expiration,
                issuer: provider.issuer,
            });
            const [header, payload] = jwt.split('.');
            const signature = base64url_1.default(crypto_1.randomBytes(64));
