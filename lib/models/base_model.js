...
import * as opaque from './formats/opaque.js';
...
export default function getBaseModel(provider) {
  ...
  class BaseModel {
    constructor(input = {}) {
      const payload = pickPayload(this.constructor, input);
      const { jti, kind } = payload;
    ...
    generateTokenId() {
      return opaque.generateTokenId(provider, this);
    }

    async getValueAndPayload() {
      return opaque.getValueAndPayload(provider, this);
    }

    static async verify(stored, options = {}) {
      return opaque.verify(provider, stored, options);
    }
