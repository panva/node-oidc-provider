...
function storageOptions(expiresIn) {
  if (typeof expiresIn === 'number') {
    return { maxAge: expiresIn * 1000 };
  }
  return undefined;
}
...
class MemoryAdapter {
  ...
  async upsert(id, payload, expiresIn) {
    const key = this.key(id);
    const options = storageOptions(expiresIn);
    ...
    if (grantable.has(this.model) && grantId) {
      ...
      const expiresAt = typeof expiresIn === 'number' ? Date.now() + (expiresIn * 1000) : Infinity;
