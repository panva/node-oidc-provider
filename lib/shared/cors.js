...
function corsMiddleware({ allowMethods, maxAge, exposeHeaders }) {
  ...
  const expose = typeof exposeHeaders === 'function' ? exposeHeaders : list(exposeHeaders);
  ...
  return async (ctx, next) => {
    ...
    if (ctx.method !== 'OPTIONS') {
      ...
      const headers = typeof expose === 'function' ? list(expose(ctx)) : expose;
      if (headers) ctx.set('Access-Control-Expose-Headers', headers);
      return next();
    }
