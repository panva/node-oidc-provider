...
module.exports = function tokenClientAuth(provider, endpoint) {
  ...
  const pkceEnabled = instance(provider).configuration('features.pkce');

  return function* tokenAuth(next) {
    const params = this.oidc.params;
    const possibleSkip = pkceEnabled && this.oidc.client.applicationType === 'native' &&
    ...
    switch (this.oidc.client.tokenEndpointAuthMethod) {
      ...
      case 'client_secret_post':
        ...
        if (possibleSkip && !params.client_secret) break;
      ...
      default: { // client_secret_basic
        ...
        if (possibleSkip && !auth.clientSecret) break;
