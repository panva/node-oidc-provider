...
module.exports = function tokenAuth(provider) {
  const tokenJwtAuth = getJWTAuthMiddleware(provider);
  const authParams = new Set(['client_id']);

  instance(provider).configuration('tokenEndpointAuthMethods').forEach((method) => {
    switch (method) {
    ...
    middleware: [
      ...
      async function auth(ctx, next) {
        const {
          params,
          client: {
            tokenEndpointAuthMethod: clientMethod,
        ...
        if (!methods.includes(clientMethod)) {
          throw new InvalidClientAuth('the registered client token_endpoint_auth_method does not match the provided auth mechanism');
        }
        ...
        switch (clientMethod) { // eslint-disable-line default-case
          ...
          case 'client_secret_jwt':
            ...
            await tokenJwtAuth(
              ...
              signingAlg ? [signingAlg] : instance(provider).configuration('tokenEndpointAuthSigningAlgValues').filter((alg) => alg.startsWith('HS')),
            );
