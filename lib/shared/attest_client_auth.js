...
export default async function attestationClientAuth(ctx) {
  ...
  const dpopFallback = (ctx.oidc.route === 'token' || ctx.oidc.route === 'pushed_authorization_request') && !!ctx.get('dpop') && !pop;
  ...
  } else if (dpopFallback) {
    const dPoP = await dpopValidate(ctx);
    const expected = await jose.calculateJwkThumbprint(verifiedAttestation.payload.cnf.jwk);
    if (dPoP.thumbprint !== expected) {
      throw new InvalidClientAuth("dpop key doesn't match the oauth-client-attestation key");
    }
    verifiedPoP = {
      protectedHeader: dPoP.protectedHeader,
