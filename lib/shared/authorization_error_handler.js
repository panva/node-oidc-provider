...
import { oneRedirectUriClients } from '../actions/authorization/response_parameters.js';
...
export default async function authorizationErrorHandler(ctx, next) {
  const { provider } = ctx.oidc;
  try {
    await next();
  } catch (caught) {
    ...
    if (
      ...
      // see the note in actions/authorization/respond.js
      await handler(ctx, safe(params.redirect_uri), omitBy(out, (value) => value === undefined));
    }
  }
}
