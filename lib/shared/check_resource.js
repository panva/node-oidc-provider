import assertProviderContext from '../helpers/assert_provider_context.js';
import { resourceServer as validateResourceServer } from '../helpers/configuration_result.js';
...
export async function resolveRequestedResources(provider, ctx) {
  ...
  if (!enabled) {
    filterStatics(ctx);
    return Object.values(resourceServers);
  }
  ...
  for (const identifier of resource) {
    ...
    const resourceServer = validateResourceServer(
      await getResourceServerInfo(ctx, identifier, client),
    );
    resourceServers[identifier] = new provider.ResourceServer(identifier, resourceServer);
  }
...
export default async function checkResource(ctx, next) {
  await resolveRequestedResources(ctx.oidc.provider, ctx);
  return next();
}
