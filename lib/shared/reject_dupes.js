...
function exceptMap([key, value]) {
  if (Array.isArray(value) && !this.includes(key)) {
    return key;
  }
  return undefined;
}
...
async function rejectDupes(ctx, next) {
  ...
  if (dupes.some(Boolean)) {
    throw new InvalidRequest(`parameters must not be provided twice. (${dupes.filter(Boolean).join(',')})`);
  }
...
module.exports.except = (ctx, next, ...except) => rejectDupes.call({ except }, ctx, next);
