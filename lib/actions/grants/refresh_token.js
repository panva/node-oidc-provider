...
const { InvalidGrant, InvalidScope, InvalidTarget } = require('../../helpers/errors');
...
module.exports.handler = async function refreshTokenHandler(ctx, next) {
  ...
  if (refreshToken.clientId !== client.clientId || grant.clientId !== client.clientId) {
  ...
  let resource;
  if (!ctx.oidc.params.resource && (!userinfo.enabled || !scope.has('openid'))) {
    resource = refreshToken.resource;
  } else {
  ...
  if (resource && !refreshToken.resourceIndicators.has(resource)) {
