...
export const handler = async function refreshTokenHandler(ctx) {
  ...
  if (refreshToken.consumed) {
    ...
    if (!refreshToken.isValid) {
      const gracePeriodSeconds = instance(ctx.oidc.provider).configuration.refreshTokenGracePeriodSeconds;
      
      if (gracePeriodSeconds && gracePeriodSeconds > 0) {
        // Grace period enabled: only destroy this specific token, preserve other valid tokens
        await refreshToken.destroy();
      } else {
        // No grace period: maintain original strict security behavior (revoke entire grant)
        await Promise.all([
    ...
    // If we reach here, the token is consumed but still valid due to grace period
  }
  ...
  if (
    ...
    // Only consume the token if it's not already consumed
    if (!refreshToken.consumed) {
      await refreshToken.consume();
    }
    // Note: For already consumed tokens, we proceed to create a new rotated token.
    // This handles the grace period scenario by allowing the consumed token to be
    // used again (validation handled in the isValid check earlier).
