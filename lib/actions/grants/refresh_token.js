  ...
  if (refreshToken.consumed) {
    ...
    if (!refreshToken.isValid) {
      ...
      if (revokeEntireGrantAfterGracePeriod) {
        ...
        // Only invalidate this specific token
        await refreshToken.destroy();
        throw new InvalidGrant('refresh token already used');
      }
    }
    // If we reach here, the token is consumed but still valid due to grace period
    // Emit an event to track this grace period usage
    ctx.oidc.provider.emit('refresh_token.reused_within_grace_period', ctx, refreshToken);
  }
