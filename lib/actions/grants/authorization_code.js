...
module.exports.handler = function getAuthorizationCodeHandler(provider) {
  ...
  return async function authorizationCodeResponse(ctx, next) {
    ...
    // });
    const code = await provider.verifyAuthorizationCode(ctx.oidc.params.code, ctx.oidc.client.clientId);
    ...
    const at = new AccessToken({
      ...
      scope: code.scope,
      sid: code.sid,
      astroAccountLink: code.astroAccountLink,
      profileId: code.profileId,
    });

    at.setAudiences(await audiences(ctx, account.accountId, at, 'access_token', code.scope));
    at.setPermissions(await permissions(ctx, account.accountId, at, 'access_token', code.scope));
    ...
    if (grantPresent && (alwaysIssueRefresh || code.scope.split(' ').includes('offline_access'))) {
      ...
      if (code.exp) {
        data.exp = code.refreshTokenExpiry;
        data.refreshTokenExpiry = code.refreshTokenExpiry;
      }
    ...
    // TODO hit link api to determine whether this user already link or not, and put in scope or new field in id token
    ...
    token.set('astroAccountLink', code.astroAccountLink);
    token.set('profileId', code.profileId);
