...
const getAuthorizationCodeHandlerDebug = require('debug')('oidc-provider:getAuthorizationCodeHandler');
...
module.exports.handler = function getAuthorizationCodeHandler(provider) {
  ...
  return async function authorizationCodeResponse(ctx, next) {
    ...
    getAuthorizationCodeHandlerDebug(`refresh_token grantPresent = ${grantPresent} alwaysIssueRefresh = ${alwaysIssueRefresh} code.scope = ${code.scope}`)

    if (grantPresent && (alwaysIssueRefresh || code.scope.split(' ').includes('offline_access'))) {
      getAuthorizationCodeHandlerDebug(`issuing refresh token`)
      ...
      getAuthorizationCodeHandlerDebug(`saved refreshToken grantId=${code.grantId}`)
    }
