...
module.exports = {
  get: async function checkSessionIframe(ctx, next) {
    const { keepHeaders } = instance(ctx.oidc.provider).configuration('features.sessionManagement');
...
function base64url (input) {
  return btoa(String.fromCharCode.apply(0, new Uint8Array(input)))
    .replace(/=/g, '')
    .replace(/\\+/g, '-')
    .replace(/\\//g, '_')
}
...
function shab64u(clientId, origin, state, salt) {
  var input = clientId + ' ' + origin + ' ' + state;
...
function calculate(clientId, origin, actual, salt, cb) {
  try {
    ...
    if (opbs) {
      shab64u(clientId, origin, opbs, salt).then(function (expected) {
        if (actual === expected) {
          stat = 'unchanged';
        }
      ...
      document.hasStorageAccess().then(function (hasAccess) {
        if (!hasAccess) {
          cb('error', new Error('document.hasStorageAccess() is false'));
        } else {
          cb(stat);
        }
      }, cb.bind(undefined, 'error'));
...
function receiveMessage(e) {
  ...
  check(clientId, e.origin, sessionStr, salt, function (stat, err) {
    if (err) {
      console.log(err);
    }
