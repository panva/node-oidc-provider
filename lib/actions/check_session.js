...
(function () {
  ${thirdPartyCheckUrl ? 'var thirdPartyCookies = true;' : ''}
  ...
  function receiveMessage(e) {
    ...
    ${
  thirdPartyCheckUrl ? `if (e.data === 'MM:3PCunsupported') {
  ...
  function getOPBrowserState(clientId) {
    ...
    ${thirdPartyCheckUrl ? "if (!thirdPartyCookies && !cookie) throw new Error('third party cookies are most likely blocked');" : ''}
    return cookie;
  }
...
${thirdPartyCheckUrl ? `<iframe src="${thirdPartyCheckUrl}" style="display:none" />` : ''}
