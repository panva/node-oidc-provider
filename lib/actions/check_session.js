...
function getOPBrowserState(clientId) {
  var cookie = readCookie('${await ctx.oidc.provider.cookieName(ctx, 'state')}.' + clientId);
  if (cookie === null) {
    cookie = readCookie('${await ctx.oidc.provider.cookieName(ctx, 'state')}.' + clientId + '.legacy');
  }
  return cookie;
}
