...
module.exports = provider => async function checkClaims(ctx, next) {
  ...
  if (params.claims !== undefined && instance(provider).configuration('features.claimsParameter')) {
    if (params.response_type === 'none') {
      ctx.throw(new InvalidRequestError('claims parameter should not be combined with response_type none'));
    }
    ...
    try {
      ...
      ctx.throw(new InvalidRequestError('could not parse the claims parameter JSON'));
    }

    if (!isPlainObject(claims)) {
      ctx.throw(new InvalidRequestError('claims parameter should be a JSON object'));
    }

    if (claims.userinfo === undefined && claims.id_token === undefined) {
      ctx.throw(new InvalidRequestError('claims parameter should have userinfo or id_token properties'));
    }

    if (claims.userinfo !== undefined && !isPlainObject(claims.userinfo)) {
      ctx.throw(new InvalidRequestError('claims.userinfo should be an object'));
    }

    if (claims.id_token !== undefined && !isPlainObject(claims.id_token)) {
      ctx.throw(new InvalidRequestError('claims.id_token should be an object'));
    }

    if (params.response_type === 'id_token' && claims.userinfo) {
      ctx.throw(new InvalidRequestError('claims.userinfo should not be used if access_token is not issued'));
    }
