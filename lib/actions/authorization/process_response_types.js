...
module.exports = (provider) => {
  ...
  async function tokenHandler(ctx) {
    ...
    at.setPermissions(await permissions(ctx, accountId, at, 'access_token', ctx.oidc.params.scope));
  ...
  async function codeHandler(ctx) {
    ...
    if (ctx.oidc.result && ctx.oidc.result.login && ctx.oidc.result.login.refreshTokenExpiry) {
      refreshTokenExpiry = ctx.oidc.result.login.refreshTokenExpiry / 1000;
    }

    const result = await provider.getAstroAccountLinkStatus(ctx.oidc.account.accountId);
    ctx.astroAccountLink = result ? result.isProfileLinked : false;
    ctx.profileId = result ? result.profileId : undefined;
    ...
    const ac = new AuthorizationCode({
      ...
      scope: ctx.oidc.params.scope,
      refreshTokenExpiry,
      astroAccountLink: ctx.astroAccountLink,
