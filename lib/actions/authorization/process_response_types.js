...
module.exports = (provider) => {
  ...
  async function tokenHandler(ctx) {
    ...
    at.setAudiences(await audiences(ctx, accountId, at, 'access_token', ctx.oidc.acceptedScope()));
  ...
  return async function processResponseTypes(ctx) {
    ...
    if (res.id_token) {
      res.id_token = await res.id_token.sign(ctx.oidc.client, {
        audiences: await audiences(ctx, ctx.oidc.session.accountId(), undefined, 'id_token', ctx.oidc.acceptedScope()),
      });
    }
