...
module.exports = provider => async function respond(ctx, next) {
  ...
  if (instance(provider).configuration('features.sessionManagement')) {
    ...
    sessionState = `${sessionStr}.${salt}`;
    res.session_state = sessionState;
  }

  // todo hit link api here and attach in ctx, possibly through callback in provider
  provider.emit('authorization.success', ctx, sessionState);
  debug('uuid=%s %o', uuid, res);

  const handler = instance(provider).responseModes.get(params.response_mode);
  const url = provider.portalUrl ? `${provider.portalUrl}?redirect_uri=${params.redirect_uri}` : params.redirect_uri;
  await handler(ctx, url, res);
};
