...
module.exports = (provider) => {
  ...
  return async function interactions(ctx, next) {
    ...
    if (interaction) {
      ...
      try {
        ctx.assert(!ctx.oidc.promptPending('none'), interaction.error, {
      ...
      ctx.cookies.set(provider.cookieName('interaction'), stringify({
        returnTo,
        interaction,
        uuid: ctx.oidc.uuid,
      ...
      ctx.cookies.set(provider.cookieName('resume'), stringify(ctx.oidc.params),
