...
import { boolean, positiveInteger } from '../../helpers/configuration_result.js';
...
export default async function interactions(ctx, next) {
  const { oidc } = ctx;
  let failedCheck;
  let prompt;
  ...
  for (const { name, checks, details: promptDetails } of policy) {
    let results = (await Promise.all([...checks].map(async ({
      ...
      if (boolean(
        await check(ctx),
        `interactions.policy.${name}.checks.${reason}.check`,
      )) {
        return {
  ...
  const resumeRouteName = /^(code|device)_/.test(oidc.route) ? 'device_resume' : 'resume';
  ...
  ttl = positiveInteger(ttl, 'ttl.Interaction');
...
export function interactionEmit(ctx, next) {
