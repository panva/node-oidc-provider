...
module.exports = (provider) => {
  ...
  return function* interactions(next) {
    ...
    if (interaction) {
      ...
      const returnTo = ctx.urlFor('resume', { grant: ctx.uuid });

      this.cookies.set(provider.cookieName('interaction'), j({
        returnTo,
        interaction,
        uuid: ctx.uuid,
        params: ctx.params,
      }), Object.assign({}, cookieOptions, { path: url.parse(destination).pathname }));

      this.cookies.set(provider.cookieName('resume'), j(ctx.params),
        Object.assign({}, cookieOptions, { path: url.parse(returnTo).pathname }));
