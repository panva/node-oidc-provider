 ...
 */
module.exports = async function checkScope(PARAM_LIST, ctx, next) {
  const { scopes: statics, ignoreOfflineAccessScope } = instance(ctx.oidc.provider).configuration();
   ...
   *
   * MUST ensure that the prompt parameter contains consent unless other conditions for processing
   *  the request permitting offline access to the requested resources are in place; unless one or
   *  both of these conditions are fulfilled, then it MUST ignore the offline_access request,
   *
  ...
  if (scopes.includes('offline_access')) {
    if (
      (PARAM_LIST.has('prompt') && (await ignoreOfflineAccessScope(ctx, client, prompts)))
