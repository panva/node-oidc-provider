...
module.exports = async function processRequestObject(PARAM_LIST, rejectDupesMiddleware, ctx, next) {
  ...
  if (client.requireSignedRequestObject && params.request === undefined) {
  ...
  const opts = {
    ...
    // TODO: in v7.x assert that if `typ` is present it is oauth-authz-req+jwt or just 'jwt'
  ...
  switch (conf('features.requestObjects.mode')) {
    case 'lax':
