...
import { getAuthorizationParams, rejectAuthorizationDupes } from './request_parameters.js';
 ...
 */
export default async function processRequestObject(ctx, next) {
  const PARAM_LIST = getAuthorizationParams(ctx);
  ...
  if (features.encryption.enabled && params.request.split('.').length === 5) {
    ...
    try {
      ...
      throw new InvalidRequestObject('could not decrypt request object', { cause });
    }
  }
  ...
  try {
    ...
    throw new InvalidRequestObject('could not parse Request Object', { cause });
  }
  ...
  const request = Object.entries(payload).reduce((acc, [key, value]) => {
    if (PARAM_LIST.has(key)) {
      if (key === 'claims' && isJsonObject(value)) {
  ...
  rejectAuthorizationDupes({ oidc: { params: request, provider: ctx.oidc.provider } }, () => {});
  ...
  try {
    ...
    throw new InvalidRequestObject('Request Object claims are invalid', { cause });
  }
  ...
  if (pushedRequestObject) {
    ({ trusted } = ctx.oidc.entities.PushedAuthorizationRequest);
  } else {
    try {
      ...
      throw new InvalidRequestObject('could not validate Request Object', { cause });
    }
  }
  ...
  Object.keys(params).forEach((key) => {
    if (key in request) {
      ...
      params[key] = request[key] || undefined;
    } else {
 ...
 */
export function stripOutsideJarParams(ctx, next) {
