...
async function clientForLogout(ctx, clientId) {
  try {
    return await ctx.oidc.provider.Client.find(clientId);
  } catch (err) {
    if (err instanceof InvalidClient) {
      return undefined;
    }
    throw err;
  }
  ...
  async function endSessionChecks(ctx, next) {
    ...
    if (params.id_token_hint) {
      try {
        ...
        throw new InvalidRequest('could not decode id_token_hint', undefined, { cause });
      }
...
export const confirm = [
  ...
  async function endSession(ctx) {
    ...
    if (backchannelLogout.enabled) {
      ...
      for (const clientId of clientIds) {
        if (params.logout || clientId === state.clientId) {
          const client = await clientForLogout(ctx, clientId);
          if (client) {
    ...
    if (state.clientId) {
      ctx.oidc.entity('Client', await clientForLogout(ctx, state.clientId));
    }
