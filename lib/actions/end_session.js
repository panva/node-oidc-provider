...
module.exports = function endSessionAction(provider) {
  ...
  async function loadClient(ctx, clientId) {
    ...
    if (!client) {
      ctx.throw(new InvalidClient('unrecognized azp or aud claims'));
    }
    return client;
  }
      ...
      async function endSessionChecks(ctx, next) {
        ...
        if (params.id_token_hint) {
          ...
          try {
            ...
            ctx.throw(new InvalidRequest(`could not decode id_token_hint (${err.message})`));
          }
          ...
          try {
            ...
            ctx.throw(new InvalidRequest(`could not validate id_token_hint (${err.message})`));
          }

          if (params.post_logout_redirect_uri) {
            if (!client.postLogoutRedirectUriAllowed(params.post_logout_redirect_uri)) {
              ctx.throw(new InvalidRequest('post_logout_redirect_uri not registered'));
            }
          }
    ...
    post: compose([
      ...
      async function checkLogoutToken(ctx, next) {
        if (!ctx.oidc.session.logout) {
          ctx.throw(new InvalidRequest('could not find logout details'));
        }
        if (ctx.oidc.session.logout.secret !== ctx.oidc.params.xsrf) {
          ctx.throw(new InvalidRequest('xsrf token invalid'));
        }
        await next();
      },
