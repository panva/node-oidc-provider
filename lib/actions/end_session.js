    ...
    async function endSession(ctx, next) {
      ...
      if (params.logout) {
        ...
        if (sessionManagement.enabled) {
          ...
          const STATES = new RegExp(`${await ctx.oidc.provider.cookieName(ctx, 'state')}\\.[^=]+=`, 'g');
        ...
        ssHandler.set(
          ctx.oidc.cookies,
          await ctx.oidc.provider.cookieName(ctx, 'session'),
          null,
          opts,
        );
        ...
        if (sessionManagement.enabled) {
          ssHandler.set(
            ctx.oidc.cookies,
            `${await ctx.oidc.provider.cookieName(ctx, 'state')}.${state.clientId}`,
            null,
            opts,
          );
        }
