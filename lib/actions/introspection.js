...
import { findToken } from '../helpers/token_find.js';
...
export default [
  ...
  function assembleTokenParams(ctx, next) {
    const params = new Set(['token', 'token_type_hint', ...getClientAuthParams(ctx)]);
    return paramsMiddleware(params, ctx, next);
  },
  ...clientAuth,
  ...
  async function jwtIntrospectionResponse(ctx, next) {
    ...
    const { features: { jwtIntrospection } } = instance(provider).configuration;
    if (jwtIntrospection.enabled) {
      ...
      if ((encrypt || sign) && accepts === JWT) {
        const token = new provider.IdToken({}, { ctx });
        token.extra = {
  ...
  async function renderTokenResponse(ctx) {
    ...
    const {
      pairwiseIdentifier,
      features: { introspection: { allowedPolicy }, richAuthorizationRequests },
    } = instance(provider).configuration;

    ctx.body = { active: false };

    const token = await findToken(provider, params.token, params.token_type_hint);
    ...
    if (!boolean(
      await allowedPolicy(ctx, ctx.oidc.client, token),
    ...
    Object.assign(ctx.body, {
      ...
      authorization_details: token.rar
        ? normalizeAuthorizationDetails(
          await richAuthorizationRequests.authorizationDetailsForIntrospection(ctx, token),
        ) : undefined,
