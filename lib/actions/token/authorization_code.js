...
module.exports.handler = function getAuthorizationCodeHandler(provider) {
  ...
  return async function authorizationCodeResponse(ctx, next) {
    ...
    ctx.assert(code, new InvalidGrantError('authorization code not found'));
    ctx.assert(!code.isExpired, new InvalidGrantError('authorization code is expired'));
    ...
    try {
      ctx.assert(!code.consumed,
        new InvalidGrantError('authorization code already consumed'));
    ...
    ctx.assert(code.clientId === ctx.oidc.client.clientId,
      new InvalidGrantError('authorization code client mismatch'));

    ctx.assert(code.redirectUri === ctx.oidc.params.redirect_uri,
      new InvalidGrantError('authorization code redirect_uri mismatch'));

    const account = await provider.Account.findById(ctx, code.accountId);

    ctx.assert(account,
      new InvalidGrantError('authorization code invalid (referenced account not found)'));
