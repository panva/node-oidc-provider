  ...
  return compose([
    ...
    function * checkClaims(next) {
      ...
      if (params.max_age || this.oidc.client.requireAuthTime) {
        this.oidc.claims = this.oidc.claims || /* istanbul ignore next */ {};
    ...
    function * loadAccount(next) {
      ...
      if (accountId) {
        ...
        account = yield Account.findById(accountId);
      }
    ...
    function * interactions(next) {
      ...
      const loginInteractions = [
        ...
        () => {
          const missed = _.find(this.oidc.prompts, (prompt) => {
            if (prompt !== 'none' && this.oidc.prompt(prompt)) {
              return true;
            }
            return false;
          });
      ...
      if (interaction) {
        ...
        const interactionPath = provider.configuration('interactionPath')(this.oidc.uuid);
        const cookieOptions = provider.configuration('cookies.short');

        this.cookies.set('_grant', j({
          details: interaction,
          params: this.oidc.params,
        }), Object.assign({ path: interactionPath }, cookieOptions));

        this.cookies.set('_grant', j(this.oidc.params), Object.assign({
          path: this.oidc.pathFor('resume', {
