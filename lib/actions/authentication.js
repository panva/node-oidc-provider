...
const mask = require('../helpers/claims_mask');
...
const body = bodyMiddleware({
  only: 'application/x-www-form-urlencoded',
  raise: true,
});

const getParams = paramsMiddleware({
...
module.exports = function authenticationAction(provider) {
  ...
  return compose([
    ...
    function * decodeRequest(next) {
      ...
      const client = yield loadClient.call(this, params.client_id);
    ...
    function * checkResponseType(next) {
      ...
      const params = this.oidc.params;
      const supported = provider.configuration.responseTypesSupported;
    ...
    function * checkClaims(next) {
      ...
      if (acrValues) {
        ...
        _.merge(this.oidc.claims, {
          id_token: {
            acr: {
              values: Array.isArray(acrValues) ? acrValues : acrValues.split(' '),
            },
          },
        });
      }
    ...
    function * interactions(next) {
      ...
      const loginInteractions = [
        ...
        () => {
          if (_.has(this.oidc.claims, 'id_token.sub.value')) {
            const subject = ClaimsMask.sub(this.oidc.accountId, this.oidc.client.sectorIdentifier);
      ...
      if (!interaction && this.oidc.params.id_token_hint !== undefined) {
        let decoded;
        const actualSub = ClaimsMask.sub(this.oidc.accountId, this.oidc.client.sectorIdentifier);
    ...
    function * handleSuccess() {
      ...
      if (out.access_token && out.id_token) {
        out.id_token.extra = { at_hash: out.access_token };
      }

      if (out.code && out.id_token) {
        out.id_token.extra = { c_hash: out.code };
      }
