...
export default [
  ...
  async function validateAccessToken(ctx, next) {
    ...
    if (!scopes.has('bank_accounts')) {
      throw new InsufficientScope('access token missing bank_accounts scope', 'bank_accounts');
    }
  ...
  async function respond(ctx, next) {
    const claims = filterClaims(ctx.oidc.accessToken.claims, 'bank_accounts', ctx.oidc.grant);
    ...
    const bankAccounts = await ctx.oidc.account.bankAccounts('bank_accounts', scope, claims, rejected);
    ctx.body = bankAccounts;
