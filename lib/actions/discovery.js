...
export default function discovery(ctx) {
  ...
  if (features.attestClientAuth.enabled) {
    ...
    ctx.body.client_attestation_signing_alg_values_supported = configuration.attestSigningAlgValues;
    ctx.body.client_attestation_pop_signing_alg_values_supported = configuration.attestSigningAlgValues;
    switch (features.attestClientAuth.additionalSecuritySignal) {
      case 'optional':
        ctx.body.client_attestation_pop_methods_supported = ['attestation_pop_jwt', 'none'];
        break;
      case 'required':
        ctx.body.client_attestation_pop_methods_supported = ['attestation_pop_jwt'];
        break;
      default:
    }
  }
