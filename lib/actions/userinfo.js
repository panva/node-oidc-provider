...
module.exports = [
  ...
  async function validateAccessToken(ctx, next) {
    ...
    if (dPoP) {
      const unique = await ctx.oidc.provider.ReplayDetection.unique(accessToken.clientId, dPoP.jti, dPoP.iat + instance(ctx.oidc.provider).configuration('features.dPoP.iatTolerance'));
