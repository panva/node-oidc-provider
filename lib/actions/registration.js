  ...
  return {
    ...
    put: compose([
      ...
      async function forbiddenFields(ctx, next) {
        ...
        if (hit) {
          ctx.throw(new InvalidRequestError(`request MUST NOT include the "${hit}" field`));
        }
      ...
      async function metaChecks(ctx, next) {
        ...
        if (hit) {
          ctx.throw(new InvalidRequestError(`${hit} must be provided`));
        }
        await next();
      },

      async function equalChecks(ctx, next) {
        ctx.assert(ctx.oidc.body.client_id === ctx.oidc.client.clientId, new InvalidRequestError(
        ...
        if (ctx.oidc.body.client_secret) {
          const clientSecretValid = constantEquals(
            new Buffer(ctx.oidc.body.client_secret, 'utf8'),
            new Buffer(ctx.oidc.client.clientSecret, 'utf8'),
            1000);

          ctx.assert(clientSecretValid, new InvalidRequestError(
      ...
      async function clientUpdateResponse(ctx, next) {
        if (ctx.oidc.client.noManage) {
          throw new InvalidRequestError('this client is not allowed to update its records',
            403);
        }
