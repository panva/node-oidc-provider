...
const JWK_REQUIREMENTS = new Map([
  ['RSA', { public: ['n', 'e'], private: ['d', 'p', 'q', 'dp', 'dq', 'qi', 'oth'] }],
  ['EC', { public: ['crv', 'x', 'y'], private: ['d'] }],
  ['OKP', { public: ['crv', 'x'], private: ['d'] }],
  ['AKP', { public: ['alg', 'pub'], private: ['priv'] }],
]);
...
async function verifyKeyAttestation(
  ...
  let key;
  try {
    key = await openid4vci.getKeyAttestationSignaturePublicKey(
      ctx,
      claims.iss,
      header,
      ctx.oidc.client,
  ...
  for (const jwk of payload.attested_keys) {
    ...
    try {
      const requirements = JWK_REQUIREMENTS.get(jwk.kty);
      if (!requirements) {
        if (typeof jwk.kty === 'string' && jwk.kty && jwk.kty !== 'oct') {
          continue;
        }
        throw new TypeError('JWK does not identify a public asymmetric key type');
      }
      if (requirements.private.some((member) => Object.hasOwn(jwk, member))) {
        throw new TypeError('JWK is not a public asymmetric key');
      }
      if (!requirements.public.every((member) => (
        typeof jwk[member] === 'string'
        && jwk[member].length !== 0
      ))) {
        throw new TypeError('JWK public key members must be non-empty strings');
      }
    } catch (cause) {
      throw new InvalidProof(
        `${errorPrefix} attested_keys entries must be public asymmetric JWKs`,
        { cause },
      );
    }
  }
  ...
  if (payload.certification !== undefined) {
    ...
    if (!URL.canParse(payload.certification)) {
      throw new InvalidProof(`${errorPrefix} certification must be a URL`);
    }
  }
...
async function verifyKeyAttestationHeader(
  ...
  const attestedThumbprints = await Promise.all(
    result.attestedKeys
      .filter((jwk) => JWK_REQUIREMENTS.has(jwk.kty))
      .map((jwk) => calculateJwkThumbprint(jwk)),
  );
...
async function embeddedProofJwk(protectedHeader, token) {
  const keyReferences = ['kid', 'jwk', 'x5c']
    .filter((parameter) => Object.hasOwn(protectedHeader, parameter));

  if (keyReferences.length !== 1) {
    throw new InvalidProof('jwt proof must use exactly one of kid, jwk, or x5c');
  }

  return EmbeddedJWK(protectedHeader, token);
}

async function verifyJwtProof(proof, issuer, clientId, supportedAlgs, cNonceChallenges) {
  ...
  let payload;
  try {
    ({ payload } = await jwtVerify(proof, embeddedProofJwk, {
    ...
    if (cause instanceof InvalidProof) {
      throw cause;
    }
  ...
  if (typeof payload.aud !== 'string') {
    throw new InvalidProof('jwt proof aud must be a string');
  }

  if (payload.iss !== undefined && payload.iss !== clientId) {
    throw new InvalidProof('jwt proof iss must equal client_id');
  }
...
export default [
  ...
  async function issueCredential(ctx) {
    ...
    if (hasCredentialIdentifier) {
      ...
      if (!Object.hasOwn(credentialConfigurationsSupported, credentialConfigurationId)) {
    ...
    const policyAllowed = boolean(
      await openid4vci.credentialConfigurationPolicy(ctx, {
    ...
    if (!hasCredentialIdentifier) {
      ...
      if (!credentialConfiguration.scope) {
        ...
        // https://www.rfc-editor.org/info/rfc9396/#section-2
        const authorized = (
