...
async function verifyKeyAttestation(
  ...
  try {
    ...
    throw new InvalidProof(`${errorPrefix} must be a valid JWT`);
  }
  ...
  const key = await openid4vci.getKeyAttestationSignaturePublicKey(
    ...
    throw new InvalidProof(`${errorPrefix} signature verification failed`);
  });
  ...
  try {
    ...
    if (err?.code === 'ERR_JWT_CLAIM_VALIDATION_FAILED' && err?.claim === 'typ') {
      throw new InvalidProof(`${errorPrefix} typ must be key-attestation+jwt`);
    }

    if (err?.code === 'ERR_JWT_CLAIM_VALIDATION_FAILED' && err?.claim === 'iat') {
      throw new InvalidProof(`${errorPrefix} iat must be present`);
    }

    if (err?.code === 'ERR_JWT_CLAIM_VALIDATION_FAILED' && err?.claim === 'exp') {
      throw new InvalidProof(`${errorPrefix} exp must be present`);
    }

    if (err?.code === 'ERR_JWT_CLAIM_VALIDATION_FAILED' && err?.claim === 'attested_keys') {
      throw new InvalidProof(`${errorPrefix} attested_keys must be present`);
    }

    if (err?.code === 'ERR_JWT_EXPIRED') {
      throw new InvalidProof(`${errorPrefix} has expired`);
    }

    if (supportedAlgs && err?.code === 'ERR_JOSE_ALG_NOT_ALLOWED') {
      throw new InvalidProof(`${errorPrefix} alg is not supported for this credential configuration`);
    }
...
async function verifyJwtProof(proof, issuer, supportedAlgs, cNonceChallenges) {
  ...
  try {
    ...
    if (err?.code === 'ERR_JWS_INVALID') {
      throw new InvalidProof('jwt proof must be a compact JWS');
    }

    if (err?.code === 'ERR_JWT_CLAIM_VALIDATION_FAILED' && err?.claim === 'typ') {
      throw new InvalidProof('jwt proof typ must be openid4vci-proof+jwt');
    }

    if (err?.code === 'ERR_JWT_CLAIM_VALIDATION_FAILED' && err?.claim === 'aud') {
      throw new InvalidProof('jwt proof aud must equal credential issuer identifier');
    }

    if (err?.code === 'ERR_JWT_CLAIM_VALIDATION_FAILED' && err?.claim === 'iat') {
      throw new InvalidProof('jwt proof iat must be present');
    }

    if (err?.code === 'ERR_JWT_CLAIM_VALIDATION_FAILED' && err?.claim === 'nonce') {
      throw new InvalidProof('jwt proof nonce must be present');
    }

    if (supportedAlgs && err?.code === 'ERR_JOSE_ALG_NOT_ALLOWED') {
      throw new InvalidProof('jwt proof alg is not supported for this credential configuration');
    }

    throw new InvalidProof('jwt proof signature verification failed');
  }
...
export default [
  ...
  function validateAudience(ctx, next) {
    if (ctx.oidc.accessToken.aud !== ctx.oidc.urlFor('credential')) {
