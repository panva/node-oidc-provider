...
module.exports = class AdapterTest {
  ...
  authorizationCodeInsert() {
    const ac = new (this.provider.get('AuthorizationCode'))(this.data);
  ...
  authorizationCodeFind(code) {
    this.ac = code;
    return this.provider.get('AuthorizationCode').find(code, {
  ...
  authorizationCodeConsume(code) {
    return code.consume().then(() => this.provider.get('AuthorizationCode').find(this.ac, {
      ignoreExpiration: true,
    })).then((found) => {
  ...
  accessTokenSave() {
    const at = new (this.provider.get('AccessToken'))(
  ...
  accessTokenFind(token) {
    this.token = token;
    return this.provider.get('AccessToken').find(token, {
      ignoreExpiration: true,
    }).then((found) => {
  ...
  accessTokenDestroy(token) {
    return token.destroy().then(() => this.provider.get('AccessToken').find(this.token, {
      ignoreExpiration: true,
    })).then((found) => {
      assert.ok(!found, 'expected token not to be found');
    })
    .then(() => this.provider.get('AuthorizationCode').find(this.ac, {
      ignoreExpiration: true,
    }))
    .then((found) => {
