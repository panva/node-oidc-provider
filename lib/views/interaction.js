...
export default function interaction(it, options) {
  ...
  __eta.res += "\n";
  const missingOIDCScope = new Set(it.details.missingOIDCScope);
  ...
  __eta.res += "\n";
  const missingOIDCClaims = new Set(it.details.missingOIDCClaims);
  ...
  __eta.res += "\n";
  const missingResourceScopes = it.details.missingResourceScopes;
  if (missingResourceScopes) {
    __eta.res += "  ";
    for (const [indicator, scopes] of Object.entries(it.details.missingResourceScopes)) {
      __eta.res += "    <li>";
  ...
  if (it.params.scope?.includes("offline_access")) {
    __eta.res +=
      "  <li>\n  the client is asking to have offline access to this authorization\n    ";
    if (!it.details.missingOIDCScope?.includes("offline_access")) {
