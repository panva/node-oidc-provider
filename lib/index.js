...
class Provider extends events.EventEmitter {
  constructor(issuer, setup) {
    ...
    get('authentication', routes.authentication, session, authentication);
    post('authentication', routes.authentication, session, authentication);

    const resume = getResumeMiddleware(this);
    get('resume', `${routes.authentication}/:grant`, session, resume, authentication);
