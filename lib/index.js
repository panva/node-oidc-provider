...
const getAuthError = require('./middlewares/authentication_error_handler');
...
class Provider extends events.EventEmitter {
  constructor(issuer, setup) {
    ...
    get('authentication', routes.authentication, authError, session, authentication);
    post('authentication', routes.authentication, authError, session, authentication);

    const resume = getResumeMiddleware(this);
    get('resume', `${routes.authentication}/:grant`, authError, session, resume, authentication);
