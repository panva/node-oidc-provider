...
class Provider extends events.EventEmitter {
  constructor(issuer, setup) {
    ...
    if (Configuration.keystore) {
      Object.defineProperty(this, 'addKey', {
        value: () => {
          throw new Error('addKey is incompatible with config.keystore');
        },
      });
    }
    ...
    post(undefined, routes.authorization, authError, session, authorization);

    const resume = getResumeMiddleware(this);
    get('resume', `${routes.authorization}/:grant`, authError, session, resume,
      authorization);

    const userinfo = getUserinfo(this);
    get('userinfo', routes.userinfo, userinfo);
    post(undefined, routes.userinfo, userinfo);
    ...
    if (Configuration.features.registration) {
      ...
      if (Configuration.features.registrationManagement) {
        put(undefined, clientRoute, error(this, 'registration_update.error'), registration.put);
        del(undefined, clientRoute, error(this, 'registration_delete.error'), registration.delete);
      }
    }
    ...
    if (Configuration.features.sessionManagement) {
      ...
      post(undefined, routes.end_session, error(this, 'end_session.error'), session, endSession);
    }
  ...
  get defaultHttpOptions() {
    return instance(this).defaultHttpOptions;
  }
