...
function grantCookie(cookies) {
  try {
    return JSON.parse(cookies.get(this.cookieName('interaction'), {
...
class Provider extends events.EventEmitter {
  ...
  interactionFinished(req, res, results) {
    ...
    const cookies = this.app.createContext(req, res).cookies;
    const resumeUrl = grantCookie.call(this, cookies).returnTo;
    const path = url.parse(resumeUrl).pathname;
    const opts = Object.assign({}, instance(this).configuration('cookies.short'), { path });

    cookies.set(this.cookieName('interactionResult'), JSON.stringify(results), opts);
  ...
  interactionDetails(req) {
    ...
    return grantCookie.call(this, this.app.createContext(req).cookies);
  }
...
Object.defineProperty(Provider.prototype, 'resume', {
  value: util.deprecate(function resume(ctx, grant, result) {
    const resumeUrl = this.urlFor('resume', { grant });
    const path = url.parse(resumeUrl).pathname;
    const opts = Object.assign({ path }, instance(this).configuration('cookies.short'));

    ctx.cookies.set(this.cookieName('interactionResult'), JSON.stringify(result), opts);
...
Object.defineProperty(Provider, 'Provider', {
  get: util.deprecate(function legacyExport() {
    return this;
