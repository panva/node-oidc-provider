...
class Provider extends events.EventEmitter {
  ...
  initialize({ adapter, clients = [], keystore } = {}) {
    ...
    return initializeAdapter.call(this, adapter)
      .then(() => initializeKeystore.call(this, keystore))
  ...
  async interactionResult(req, res, result, { mergeWithLastSubmission = true } = {}) {
    ...
    if (mergeWithLastSubmission && !('error' in result)) {
      interaction.result = Object.assign({}, interaction.lastSubmission, result);
    } else {
