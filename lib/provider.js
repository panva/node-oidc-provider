...
function getInteractionIdFromCookie(req, res, providerInstance) {
  const ctx = this.app.createContext(req, res);
  return ctx.cookies.get(
    this.cookieName('interaction'),
    providerInstance.configuration.cookies.short,
  );
}

function getInteractionIdFromPath(req) {
  // The path starts with /:interactionId. We capture it so we can look up the interaction.
  // The path optionally starts with /interaction depending how the endpoints are mounted.
  // eslint-disable-next-line no-useless-escape
  const interactionIdMatches = req.url.match(/(?:\/interaction)?\/([^\/]+)\/?/);
  const interactionId = interactionIdMatches[1];
  if (!interactionId) {
    throw new SessionNotFound('No interactionID in path');
  }
  return interactionId;
}

async function getInteraction(req, res, providerInstance) {
  const enableCookielessFallback = providerInstance.configuration.cookies.enableCookielessFallback === true;
  let interactionId = getInteractionIdFromCookie.bind(this)(req, res, providerInstance);
...
export class Provider extends Koa {
  ...
  async #getInteraction(req, res) {
    return getInteraction.call(this, req, res, this.#int);
  }
