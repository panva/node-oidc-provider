...
class Provider extends events.EventEmitter {
  constructor(issuer, setup = {}, prefix, isLocal) {
    ...
    assert(setup.redirectUrl, 'Redirect URL cannot be empty');

    super();

    this.prefix = prefix;
    this.isLocal = (isLocal === 'true');
    this.verifyAuthorizationCode = setup.verifyAuthorizationCode;
    this.checkUserVerified = setup.checkUserVerified;
    this.redirectUrl = setup.redirectUrl;
    this.portalUrl = setup.portalUrl;
    this.getAstroAccountLinkStatus = setup.getAstroAccountLinkStatus;
  ...
  initialize({ adapter, clients = [], keystore } = {}) {
    ...
    return initializeKeystore.call(this, keystore)
      ...
      .then(async () => {
        await this.insertClientToDb(clients);
      })
  ...
  insertClientToDb(clients) {
    clients.forEach(async (client) => {
      ...
      const newClient = await instance(this).clientAdd(properties, true);
  ...
  initializeClients(clients) {
    this.insertClientToDb(clients);
    return initializeClients.call(this, clients);
  }
  ...
  async interactionFinished(req, res, result) {
    ...
    res.send({
      redirect: interaction.returnTo,
      partnerKey: req.details.params.client_id,
    });
