...
async function getInteraction(req, res) {
  ...
  const id = ssHandler.get(
    ctx.cookies,
    await this.cookieName(ctx, 'interaction'),
...
class Provider extends events.EventEmitter {
  ...
  async cookieName(ctx, type) {
    const cookiesNamesConfiguration = instance(this).configuration('cookies.names');

    const name = cookiesNamesConfiguration instanceof Function
      ? await cookiesNamesConfiguration(ctx, type)
      : cookiesNamesConfiguration[type];
  ...
  async setProviderSession(req, res, {
    ...
    ssHandler.set(
      ctx.oidc.cookies,
      await this.cookieName(ctx, 'session'),
      session.id,
