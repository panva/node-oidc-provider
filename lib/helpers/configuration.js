...
class Configuration {
  ...
  setAlgs(prop, values, ...features) {
    if (features.length === 0 || features.every((feature) => get(this.features, feature))) {
      this[prop] = values;
    } else {
      this[prop] = [];
    }
  ...
  defaultSigAlg() {
    ...
    this.setAlgs('requestObjectSigningAlgValues', allowList.requestObjectSigningAlgValues.slice(), 'requestObjects.enabled');
    this.setAlgs('requestObjectEncryptionAlgValues', allowList.requestObjectEncryptionAlgValues.filter(RegExp.prototype.test.bind(/^(A|dir$)/)), 'encryption.enabled', 'requestObjects.enabled');
    this.setAlgs('requestObjectEncryptionEncValues', allowList.requestObjectEncryptionEncValues.slice(), 'encryption.enabled', 'requestObjects.enabled');
