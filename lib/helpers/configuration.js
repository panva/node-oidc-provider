...
const requiredFeatureMembers = {
  attestClientAuth: ['additionalSecuritySignal'],
  ciba: ['deliveryModes'],
};

const requiredEnabledFeatureMembers = {
  deviceFlow: ['enabled', 'charset', 'mask'],
  fapi: ['enabled', 'profile'],
  openid4vci: [
    'enabled',
    'metadata',
  ...
  richAuthorizationRequests: ['enabled', 'types'],
};

function checkFeatureConfigurationOwnProperties({ features }) {
  const featureNames = [
    ...Object.keys(requiredFeatureMembers),
    ...Object.keys(requiredEnabledFeatureMembers),
  ];

  if (features && !isPlainObject(features) && ['object', 'function'].includes(typeof features)) {
    for (const name of featureNames) {
      if (!Object.hasOwn(features, name)) {
        throw new TypeError(`features.${name} must be an own property`);
      }
    }
  }

  for (const [name, members] of Object.entries(requiredFeatureMembers)) {
    const feature = features?.[name];
    if (feature && !isPlainObject(feature) && ['object', 'function'].includes(typeof feature)) {
      for (const member of members) {
        if (!Object.hasOwn(feature, member)) {
          throw new TypeError(`features.${name}.${member} must be an own property`);
        }
      }
    }
  }

  for (const [name, members] of Object.entries(requiredEnabledFeatureMembers)) {
    const feature = features?.[name];
    if (feature && !isPlainObject(feature)
        && ['object', 'function'].includes(typeof feature)) {
      if (!Object.hasOwn(feature, 'enabled')) {
        throw new TypeError(`features.${name}.enabled must be an own property`);
      }
...
function checkKeyAttestationsRequired(proofTypeConfig, prefix) {
  if (proofTypeConfig === undefined) {
    return;
  }
  if (proofTypeConfig === null) {
...
class Configuration {
  ...
  constructor(config) {
    const recognizedFeatures = { ...this.#defaults.features };
    const recognizedRichAuthorizationRequestOptions = new Set(
      Object.keys(this.#defaults.features.richAuthorizationRequests),
    );
    const input = cloneConfiguration(pick(config, ...Object.keys(this.#defaults)));
    Object.assign(this, merge({}, this.#defaults, input));

    checkFeatureConfigurationOwnProperties(this);
    featuresTypeErrorCheck(this);

    this.logDraftNotice(recognizedFeatures);
    ...
    this.checkRichAuthorizationRequests(recognizedRichAuthorizationRequestOptions);
  ...
  checkRichAuthorizationRequests(recognizedOptions) {
    const { richAuthorizationRequests } = this.features;

    for (const option of Object.keys(richAuthorizationRequests)) {
      if (!recognizedOptions.has(option)) {
        throw new TypeError(`Unknown feature configuration: richAuthorizationRequests.${option}`);
      }
    }
    ...
    for (const option of [
      ...
      const callback = richAuthorizationRequests[option];
      if (
        typeof callback !== 'function'
        || !['Function', 'AsyncFunction'].includes(callback.constructor.name)
      ) {
        throw new TypeError(`features.richAuthorizationRequests.${option} must be a function`);
      }
    }
      ...
      if (openid4vci.enabled && !Object.hasOwn(richAuthorizationRequests.types, 'openid_credential')) {
        richAuthorizationRequests.types.openid_credential = {
          validate(_ctx, detail) {
            ...
            if (!Object.hasOwn(openid4vci.credentialConfigurationsSupported, detail.credential_configuration_id)) {
              throw new InvalidAuthorizationDetails("'credential_configuration_id' must reference an entry in credential_configurations_supported");
            }
  ...
  checkAllowedJWA() {
    if (this.enabledJWA === null) {
      throw new TypeError('enabledJWA must be an object');
    }
  ...
  logDraftNotice(recognizedFeatures = getDefaults().features) {
