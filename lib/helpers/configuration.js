...
module.exports = class Configuration {
  ...
  defaultSigAlg() {
    ...
    this.setAlgs('dPoPSigningAlgValues', whitelist.dPoPSigningAlgValues.slice(), 'dPoP');
      ...
      if (this[`${endpoint}EndpointAuthMethods`]) {
        this[`${endpoint}EndpointAuthMethods`].forEach((method) => {
          if (!authMethods.has(method)) {
            throw new Error(`only supported ${endpoint}EndpointAuthMethods are [${[...authMethods]}]`);
          }
        });
      }
    });
  ...
  logDraftNotice() {
    ...
    Object.entries(this.features).forEach(([flag, { enabled, ack }]) => {
      if (!(flag in defaults.features)) {
        throw new Error(`Unknown feature configuration: ${flag}`);
      }
