...
function getKeyStore(input) {
  if (input instanceof JWKS.KeyStore) {
    return new JWKS.KeyStore(input.all());
  }
  return JWKS.KeyStore.fromJWKS(input);
}
...
you are expected to provide your own during provider#initialize');
  ...
  const keystore = getKeyStore(conf);
