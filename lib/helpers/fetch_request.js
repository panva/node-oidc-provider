...
function socketSymbolFor(target) {
  if (target === null || target === undefined) return undefined;

  return Object.getOwnPropertySymbols(target).find((s) => s.description === 'socket');
}

function destroySpecialUseSocket(target, socketSymbol = socketSymbolFor(target)) {
  const socket = socketSymbol === undefined ? undefined : target[socketSymbol];
...
function applySSRFProtection(dispatcher) {
  dispatcher.on('connect', (_origin, targets) => {
    const client = targets[targets.length - 1];
    destroySpecialUseSocket(client);
  });

  return dispatcher;
}

function createSSRFProtectedDispatcher() {
...
export default async function fetchRequest(provider, url, options) {
  ...
  if (dispatcher === undefined) {
    try {
      dispatcher = createSSRFProtectedDispatcher();
    } catch {
