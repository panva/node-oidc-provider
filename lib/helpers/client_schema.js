...
export default function getSchema(provider) {
  ...
  class Schema {
    constructor(
      ...
      const responseTypes = [...new Set(this.response_types.map((rt) => rt.split(" ")))].reduce(
        (acc, val) => [...acc, ...val],
        []
      );
      ...
      Object.entries(ENUM).forEach(([prop, fn]) => {
        ...
        if (this[prop] !== undefined) {
          ...
          if (isAry && !this[prop].every((val) => only[method](val))) {
            if (length) {
              this.invalidate(
                `${prop} can only contain ${formatters.formatList([...only], {
                  type: "disjunction",
                })}`
              );
            } else {
    ...
    webMessageUris() {
      ...
      this.web_message_uris.forEach((uri) => {
        ...
        if (!["https:", "http:", "capacitor:"].includes(protocol)) {
