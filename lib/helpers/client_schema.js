  ...
  class Schema {
    ...
    processCustomMetadata(ctx) {
      configuration.extraClientMetadata.properties.forEach((prop) => {
        const result = configuration.extraClientMetadata.validator(ctx, prop, this[prop], this);
        if (result && typeof result.then === 'function') {
          Promise.resolve(result).catch(() => {});
          throw new TypeError('extraClientMetadata.validator must be a synchronous function');
        }
      });
    }
