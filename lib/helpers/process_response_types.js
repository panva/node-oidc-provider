...
async function tokenHandler(ctx) {
  ...
  if (!token.resourceServer || token.resourceServer.accessTokenFormat === 'opaque') {
    if (boolean(await expiresWithSession(ctx, token), 'expiresWithSession')) {
...
async function codeHandler(ctx) {
  ...
  if (boolean(await expiresWithSession(ctx, code), 'expiresWithSession')) {
  ...
  if (
    ...
    code.rar = normalizeAuthorizationDetails(
      await richAuthorizationRequests.authorizationDetailsForGrantSource(ctx, code),
    );
  }
