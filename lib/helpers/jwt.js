...
const { CompactEncrypt } = require('jose/jwe/compact/encrypt'); // eslint-disable-line import/no-unresolved
const { CompactSign } = require('jose/jws/compact/sign'); // eslint-disable-line import/no-unresolved
const { compactDecrypt } = require('jose/jwe/compact/decrypt'); // eslint-disable-line import/no-unresolved
const { compactVerify } = require('jose/jws/compact/verify'); // eslint-disable-line import/no-unresolved
const { decodeProtectedHeader } = require('jose/util/decode_protected_header'); // eslint-disable-line import/no-unresolved
      ...
      if (keys.length === 0) {
        ...
        for (const key of keys) {
          try {
            ...
            let keyObject = await keystore.getKeyObject(key, header.alg);
