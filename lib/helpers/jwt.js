...
function getSignOptions(forPayload, options = {}) {
  const payload = forPayload;
  const timestamp = Math.floor(Date.now() / 1000);

  if (!options.noTimestamp) payload.iat = payload.iat || timestamp;
  if (options.expiresIn) payload.exp = timestamp + options.expiresIn;
...
class JWT {
  ...
  static sign(payload, key, alg, options) {
    ...
    return jose.JWS.createSign(
      { fields, format },
      { key, reference: key.kty !== 'oct' }).update(j(payload)).final();
  }
  ...
  static verify(jwt, keyOrStore, options = {}) {
    return createVerify(keyOrStore, jwt).catch((err) => {
      ...
      const jws = verified;
      jws.payload = JSON.parse(jws.payload);
      this.assertPayload(jws.payload, options);
      return jws;
    });
  }
