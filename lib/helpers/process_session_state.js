...
async function processSessionState(ctx, redirectUri, salt) {
  const { oidc } = ctx;
  ...
  const stateCookieName = `${await oidc.provider.cookieName(ctx, 'state')}.${clientId}`;
  ssHandler.set(
    oidc.cookies,
    stateCookieName,
    state,
...
module.exports.salted = async function saltedProcessSessionState(ctx, redirectUri) {
