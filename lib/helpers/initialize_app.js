...
module.exports = function initializeApp() {
  ...
  const userInfoCors = cors({ allowMethods: 'GET,POST', ...CORS_AUTHORIZATION });
  ...
  const tokenCors = cors({ allowMethods: 'POST', ...CORS_AUTHORIZATION });
  post('token', routes.token, tokenCors, error(this, 'grant.error'), ...token);
  ...
  if (configuration.features.revocation.enabled) {
    ...
    post('revocation', routes.revocation, tokenCors, error(this, 'revocation.error'), ...revocation);
  ...
  if (configuration.features.introspection.enabled) {
    ...
    post('introspection', routes.introspection, tokenCors, error(this, 'introspection.error'), ...introspection);
