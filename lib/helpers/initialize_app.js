...
module.exports = function initializeApp() {
  ...
  async function proxyWarning(ctx, next) {
    ...
    if (issuer.startsWith('https:') && !ctx.secure && ctx.get('x-forwarded-proto') === 'https') {
      attention.warn(`x-forwarded-proto header detected but not trusted, you must set proxy=true on the provider, see documentation for more details (${pkg.homepage}/blob/${pkg.version}/docs/configuration.md#trusting-tls-offloading-proxies)`);
      proxyWarning.pass = true;
    } else if (issuer.startsWith('https:') && !ctx.secure && !ctx.get('x-forwarded-proto')) {
      attention.warn(`x-forwarded-proto header not detected for an https issuer, you must configure your ssl offloading proxy and the provider, see documentation for more details (${pkg.homepage}/blob/${pkg.version}/docs/configuration.md#trusting-tls-offloading-proxies)`);
      proxyWarning.pass = true;
    }
