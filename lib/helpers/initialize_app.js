...
const {
  getAuthorization, userinfo, getToken, certificates, registration, getRevocation,
...
module.exports = function initializeApp() {
  ...
  const onlyGetCors = cors({ allowMethods: 'GET', maxAge: 3600 });
  ...
  const userInfoCors = cors({ allowMethods: 'GET,POST', clientBased: true, ...CORS_AUTHORIZATION });
  ...
  options('cors.userinfo', routes.userinfo, userInfoCors);

  const token = getToken(this);
  const tokenCors = cors({ allowMethods: 'POST', clientBased: true, ...CORS_AUTHORIZATION });
  post('token', routes.token, error(this, 'grant.error'), tokenCors, ...token);
  options('cors.token', routes.token, tokenCors);

  get('certificates', routes.certificates, onlyGetCors, error(this, 'certificates.error'), certificates);
  options('cors.certificates', routes.certificates, onlyGetCors);
  ...
  if (configuration.features.revocation.enabled) {
    ...
    post('revocation', routes.revocation, error(this, 'revocation.error'), tokenCors, ...revocation);
    options('cors.revocation', routes.revocation, tokenCors);
  }

  if (configuration.features.introspection.enabled) {
    const introspection = getIntrospection(this);
    post('introspection', routes.introspection, error(this, 'introspection.error'), tokenCors, ...introspection);
    options('cors.introspection', routes.introspection, tokenCors);
  }

  if (configuration.features.discovery.enabled) {
    get('discovery', discoveryRoute, onlyGetCors, error(this, 'discovery.error'), discovery);
    options('cors.discovery', discoveryRoute, onlyGetCors);
  }
