...
async function createClient(provider, properties) {
...
async function assertClientAllowed(feature, ctx, client) {
  if (!boolean(
    await feature.allowClient(ctx, client),
...
async function resolveCachedClient(provider, feature, ctx, properties) {
  const client = await createClient(provider, properties);
...
export async function resolveClientByMetadataDocument(provider, id) {
  ...
  if (!boolean(
    await feature.allowFetch(ctx, id),
  ...
  try {
    ...
    client = await createClient(provider, fetched.properties);
