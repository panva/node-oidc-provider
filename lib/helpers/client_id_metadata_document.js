...
function getCache(provider) {
  ...
  if (!entries) {
    entries = new QuickLRU({ maxSize: 100 });
...
export async function resolveClientByMetadataDocument(provider, id) {
  ...
  // Check cache
  const properties = entries.get(id);
  if (properties) {
  ...
  if (FORBIDDEN_AUTH_METHODS.has(parsed.token_endpoint_auth_method)) {
  ...
  if ('client_secret' in parsed || 'client_secret_expires_at' in parsed) {
  ...
  for (const prop of WEB_URI) {
    if (prop in parsed) {
      const uri = URL.parse(parsed[prop]);
  ...
  const client = await addClient(provider, parsed, { cimd: true });

  Object.defineProperty(client, 'clientIdMetadataDocument', { value: true });

  // Cache the valid properties
  entries.set(id, parsed, { maxAge: ttl * 1000 });
