  ...
  class OIDCContext {
    ...
    urlFor(name, opt) {
      const mountPath = (this.ctx.req.originalUrl && this.ctx.req.originalUrl.substring(
        ...
        console.log("href: " + this.ctx.href);
        console.log("provider.prefix: " + provider.prefix);
        console.log("provider.isLocal: " + provider.isLocal);

        console.log("provider.pathfor replaced: " + provider.pathFor(name, { mountPath, ...opt }).replace(/^\/+/g, ''));

        if(!!provider.isLocal){
          console.log("Result islocal true: " + url.resolve(this.ctx.href, provider.pathFor(name, { mountPath, ...opt }).replace(/^\/+/g, '')));

          return url.resolve(this.ctx.href, provider.pathFor(name, { mountPath, ...opt }).replace(/^\/+/g, ''));
        } else {
          console.log("Result islocal false: " + url.resolve(this.ctx.href, path.join(provider.prefix, provider.pathFor(name, { mountPath, ...opt }).replace(/^\/+/g, ''))));
