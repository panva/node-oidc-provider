...
import isJsonObject from './_/is_json_object.js';
  ...
  class OIDCContext extends events.EventEmitter {
    ...
    get fapiProfile() {
      if (this.#fapiProfile === null) {
        this.#fapiProfile = state.configuration.features.fapi.profile(this.ctx, this.client);
      }
    ...
    get requestParamClaims() {
      ...
      if (this.params.claims) {
        ...
        const claims = state.configuration.claimsSupported;
        if (userinfo) {
          Object.entries(userinfo).forEach(([claim, value]) => {
            if (claims.has(claim) && (value === null || isJsonObject(value))) {
    ...
    get requestParamOIDCScopes() {
      return new Set(this.params.scope?.split(' ').filter(Set.prototype.has.bind(state.configuration.scopes)));
    }
    ...
    getAccessToken({
      acceptDPoP = false,
      acceptQueryParam = state.configuration.acceptQueryParamAccessTokens
        && !state.configuration.features.fapi.enabled,
    } = {}) {
      if (this.#accessToken) {
      ...
      const dpop = acceptDPoP && state.configuration.features.dPoP.enabled && ctx.get('DPoP');
