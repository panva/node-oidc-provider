  ...
  class OIDCContext {
    ...
    urlFor(name, opt) {
      const mountPath = (this.ctx.req.originalUrl && this.ctx.req.originalUrl.substring(
        ...
        if(provider.isLocal){
          console.log("Result: " + url.resolve(this.ctx.href, provider.pathFor(name, { mountPath, ...opt }).replace(/^\/+/g, '')));

          return url.resolve(this.ctx.href, provider.pathFor(name, { mountPath, ...opt }).replace(/^\/+/g, ''));
        } else {
          console.log("Result: " + url.resolve(this.ctx.href, path.join(provider.prefix, provider.pathFor(name, { mountPath, ...opt }).replace(/^\/+/g, ''))));
