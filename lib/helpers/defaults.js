...
const DEFAULTS = {
   ...
   *   the provider instance will fail to initialize if a new version of oidc-provider bundles
   ...
   * Configure `issueRefreshToken` like so, this is how the removed `alwaysIssueRefresh` feature
   * worked
   * ```js
   ...
   *   against.
   *
   * example: Example: To enable a dynamic scope values like `api:write:{hex id}` and `api:read:{hex id}`
   ...
   *   or id_token_hint is provided by the RP
   *
   * example: validating post_logout_redirect_uri without id_token_hint
   *
   * Session Management 1.0 [lacks](https://bitbucket.org/openid/connect/issues/1032/rp-initiated-logout-proposal-for-client_id)
   * the client_id parameter, despite use-cases being there it is not part of the specification, its
   * absence means the provider cannot efficiently load a client to validate a post_logout_redirect_uri.
   * Here's how to add support for a custom client_id parameter so that your clients without
   * id_token_hint can use post_logout_redirect_uri.
   *
   * __Note:__ It is still the best recommended approach to send in the id_token_hint if client has access
   * to it.
   *
   * ```js
