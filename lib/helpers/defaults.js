...
<body>
  ...
  interactionUrl(interaction) { // eslint-disable-line no-unused-vars
    // this => koa context;
    try {
      const devInteraction = this.oidc.urlFor('interaction', { grant: this.oidc.uuid });
      if (devInteraction instanceof Error) throw devInteraction; // specific to koa-router
      return url.parse(devInteraction).pathname;
    } catch (err) {
