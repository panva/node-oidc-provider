...
function makeDefaults() {
  const defaults = {
    ...
    features: {
       ...
       *   user-facing authorization grant (e.g. Authorization Code). The token's audience MUST
       *   match the Credential Endpoint URL. The token MUST use the `opaque` format. Deployments
       *   shall use the `features.resourceIndicators` mechanism to configure the Credential
       *   Endpoint URL as a resource indicator. Use the `defaultResource` helper to detect
       *   credential-requesting authorization requests and return the Credential Endpoint URL
       *   as the resource so that the client needs not to use the `resource` parameter.
       *   Use the `useGrantedResource` helper to return so
