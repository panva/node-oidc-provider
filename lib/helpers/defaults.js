...
function makeDefaults() {
  const defaults = {
    ...
    /*
     * relaxImplicitGrantRequirement
     *
     * description: Specifies whether the authorization server shall relax the requirement that
     *   clients with 'id_token' or 'token' in their response_types must include 'implicit' in
     *   their grant_types. When set to `true`, the server allows clients to register with
     *   response types that traditionally require implicit grant without actually enabling the
     *   implicit grant type. When set to `false`, the standard OAuth 2.0 validation applies
     *   requiring 'implicit' grant_type for 'id_token'/'token' response_types.
     */
    ...
    cookies: {
      ...
      /*
       * cookies.shouldWriteCookies
       *
       * description: Optional function to determine whether cookies should be written for a given request.
       *   This function receives the Koa context and should return a boolean indicating whether cookies
       *   should be written. This allows for dynamic control over cookie writing behavior based on
       *   request context, client configuration, or other factors.
       *
       *   When not provided, defaults to checking `!cookies.doNotSet` configuration.
       *
       * @param ctx - Koa request context
       * @returns {boolean} - true if cookies should be written, false otherwise
       *
