...
async function audiences(ctx, sub, token, use) { // eslint-disable-line no-unused-vars
...
function getCertificate(ctx) { // eslint-disable-line no-unused-vars
...
/* istanbul ignore next */
function certificateAuthorized(ctx) { // eslint-disable-line no-unused-vars
...
/* istanbul ignore next */
function certificateSubjectMatches(ctx, property, expected) { // eslint-disable-line no-unused-vars
...
async function userCodeConfirmSource(ctx, form, client, deviceInfo, userCode) { // eslint-disable-line no-unused-vars, no-shadow
  // @param ctx - koa request context
...
function idFactory(ctx) { // eslint-disable-line no-unused-vars
  return nanoid();
}

function secretFactory(ctx) { // eslint-disable-line no-unused-vars
  return base64url.encodeBuffer(crypto.randomBytes(64)); // 512 base64url random bits
}

async function resourceIndicatorsAllowedPolicy(ctx, resources, client) { // eslint-disable-line no-unused-vars
...
function sessionManagementScriptNonce(ctx) { // eslint-disable-line no-unused-vars
...
function webMessageResponseModeScriptNonce(ctx) { // eslint-disable-line no-unused-vars
...
async function tokenSigningAlg(ctx, token) { // eslint-disable-line no-unused-vars
  shouldChange('formats.tokenSigningAlg', "specify the JWS Signing Algorithm the JWT Access Token will be signed with. It defaults to the provider's default id_token_signed_response_alg");
  return undefined;
}

function httpOptions(url) { // eslint-disable-line no-unused-vars
  return {
    timeout: 2500,
...
function AccessTokenTTL(ctx, token, client) { // eslint-disable-line no-unused-vars
  shouldChange('ttl.AccessToken', 'define the expiration for issued AccessToken artifacts');
  return 60 * 60; // 1 hour in seconds
}

function AuthorizationCodeTTL(ctx, token, client) { // eslint-disable-line no-unused-vars
  return 10 * 60; // 10 minutes in seconds
}

function ClientCredentialsTTL(ctx, token, client) { // eslint-disable-line no-unused-vars
  shouldChange('ttl.ClientCredentials', 'define the expiration for issued ClientCredentials artifacts');
  return 10 * 60; // 10 minutes in seconds
}

function DeviceCodeTTL(ctx, token, client) { // eslint-disable-line no-unused-vars
  shouldChange('ttl.DeviceCode', 'define the expiration for issued DeviceCode artifacts');
  return 10 * 60; // 10 minutes in seconds
}

function IdTokenTTL(ctx, token, client) { // eslint-disable-line no-unused-vars
  shouldChange('ttl.IdToken', 'define the expiration for issued IdToken artifacts');
  return 60 * 60; // 1 hour in seconds
}

function RefreshTokenTTL(ctx, token, client) {
  shouldChange('ttl.RefreshToken', 'define the expiration for issued RefreshToken artifacts');
  if (
...
function extraClientMetadataValidator(ctx, key, value, metadata) { // eslint-disable-line no-unused-vars
...
async function renderError(ctx, out, error) { // eslint-disable-line no-unused-vars
...
async function interactionsUrl(ctx, interaction) { // eslint-disable-line no-unused-vars
  return `/interaction/${ctx.oidc.uid}`;
}

async function findAccount(ctx, sub, token) { // eslint-disable-line no-unused-vars
  // @param ctx - koa request context
...
function getDefaults() {
  const defaults = {
    ...
    features: {
       ...
       *
       */
      ietfJWTAccessTokenProfile: { enabled: false, ack: undefined },
       ...
       *
       * title: [draft-ietf-oauth-par-03](https://tools.ietf.org/html/draft-ietf-oauth-par-03) - OAuth 2.0 Pushed Authorization Requests
       *
      ...
      resourceIndicators: {
         ...
         */
        allowedPolicy: resourceIndicatorsAllowedPolicy,
      },

      /*
       * features.requestObjects
       *
       * title: [Core 1.0](https://openid.net/specs/openid-connect-core-1_0.html#RequestObject) and [JWT Secured Authorization Request (JAR)](https://tools.ietf.org/html/draft-ietf-oauth-jwsreq-26) - Request Object
       *
      ...
      requestObjects: {
        ...
        mergingStrategy: {
           ...
           *
           */
          whitelist: ['code_challenge', 'nonce', 'state', 'prompt'],
        },
      },
      ...
      sessionManagement: {
         ...
         */
        scriptNonce: sessionManagementScriptNonce,
      },
     ...
     *   and `jwt-ietf` formatted tokens as top level claims as well. Returned claims may not
     ...
     * ttl
     *
     * description: description: Expirations for all token types. The value can be a number (in seconds) or a
     * synchronous function that dynamically returns value based on the context.
     *
