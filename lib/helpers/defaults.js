...
async function defaultResource(_ctx, _client, oneOf) {
  if (oneOf) return oneOf;
  return undefined;
}

async function useGrantedResource(_ctx, _source) {
  return false;
}
...
async function expiresWithSession(_ctx, source) {
...
function PreAuthorizedCodeTTL(ctx, code) {
...
async function assertJwtClientAuthClaimsAndHeader(ctx, claims, _header, _client) {
  ...
  const defaults = {
    ...
    features: {
      ...
      ciba: {
         ...
         *   according to authorization server policy. Returning `undefined` causes the request to
         *   fail because no end-user could be identified.
         *
      ...
      mTLS: {
         ...
         *   certificate used in the current request. Returning `undefined` causes authentication
         *   to fail wherever a certificate is required.
         */
        getCertificate,
         ...
         *   Certificate Authority for the requesting client. This validation is exclusively used
         *   for the `tls_client_auth` client authentication method.
         *
         ...
         *   property according to authorization server policy. This validation is exclusively used
         *   for the `tls_client_auth` client authentication method.
         *
         *   `property` is the registered `tls_client_auth_*` metadata name and `expected` is its
         *   registered value. `true` accepts the subject match; `false` rejects client
         *   authentication.
         */
      ...
      attestClientAuth: {
         ...
         *
         *   The authorization server uses the resolved key to verify the Client Attestation JWT
         *   signature. An unsupported or invalid key rejects client authentication.
         *
      ...
      introspection: {
         ...
         *
         *   `true` includes the token's active response; `false` returns the normal inactive
         *   response without revealing whether the token exists. The default permits confidential
         *   clients and only permits public clients to introspect their own tokens.
         */
       ...
       *
       *   This provider profile requires `features.resourceIndicators` and supports authorization
       *   requests whose response type contains `code` but not `token`. Deployments handling sensitive
       *   authorization details SHOULD use JAR or PAR, sanitize all consent presentation, compare
       *   string values exactly without Unicode normalization, and disclose details to clients and
       *   Resource Servers only as required by policy.
       */
      richAuthorizationRequests: {
         ...
         *   of that specific type according to authorization server policy. The validation function
         *   is responsible for rejecting unknown fields as well as missing or invalid type-specific
         *   fields with `errors.InvalidAuthorizationDetails`.
         *
         ...
         *
         * description: Specifies a helper function that shall be invoked before an AuthorizationCode
         *   or DeviceCode grant source is persisted when Rich Authorization Request details were
         *   requested or granted. The function shall apply authorization server policy to the
         *   requested and granted details and return the authorization details to store in the grant
         *   source, or undefined. An empty array is treated as undefined.
         */
        authorizationDetailsForGrantSource(ctx, source) {
          ...
          mustChange('features.richAuthorizationRequests.authorizationDetailsForGrantSource', 'apply authorization server policy to RAR details stored in grant sources');
          throw new Error(
         ...
         *
         * description: Specifies a helper function that shall be invoked before an AccessToken or
         *   ClientCredentials token is persisted whenever Rich Authorization Request details were
         *   requested or inherited from the grant source. The function shall perform type-specific
         *   grant comparison, client policy enforcement, resource-specific filtering, and any
         *   response enrichment. It shall return the exact authorization details assigned to the
         *   access token and returned from the token endpoint, or undefined. An empty array is treated
         *   as undefined. `source` is the exchanged grant source, or undefined for client credentials;
         *   `grantType` is the exact token request `grant_type` value, including full URN values. To
         *   reject client-provided authorization details, throw `errors.InvalidAuthorizationDetails`.
         */
        authorizationDetailsForAccessToken(ctx, token, source, grantType) {
          ...
          mustChange('features.richAuthorizationRequests.authorizationDetailsForAccessToken', 'apply authorization server policy to RAR details assigned to access tokens');
          throw new Error(
         ...
         *
         * description: Specifies a helper function that shall be invoked when a token containing
         *   Rich Authorization Request details is introspected. It shall apply authorization server
         *   policy for the requesting party and return the authorization details to include as the
         *   top-level `authorization_details` introspection response member, or undefined. An empty
        ...
        authorizationDetailsForIntrospection(ctx, token) {
          ...
          // - token.kind
          // - token.rar
          mustChange('features.richAuthorizationRequests.authorizationDetailsForIntrospection', 'apply authorization server policy to RAR details returned from introspection');
          throw new Error(
      ...
      resourceIndicators: {
         ...
         *
         *   `oneOf`, when present, contains the candidate resource indicators. Leaving that array
         *   unresolved causes a request that requires one target to fail with `invalid_target`.
         */
        defaultResource,
         ...
         *
         *   `true` permits the already granted resource to be selected when the request omits
         *   `resource`; `false` does not.
         *
         ...
         *
         *   A returned `accessTokenTTL` must be a positive safe integer number of seconds.
         *   Fractional, non-finite, non-positive, and unsafe integer values are rejected.
         *
      ...
      revocation: {
         ...
         *
         *   `true` permits revocation; `false` leaves the token unchanged and returns the normal
         *   revocation response. By default, a client may revoke its own tokens; a mismatched
         *   public client is denied without an error and a mismatched confidential client is
         *   rejected.
         */
      ...
      openid4vci: {
         ...
         *
         *   The returned object MUST contain a non-empty `credentials` array. The provider emits
         *   that array and the optional string `notification_id`; a missing or empty credentials
         *   array is a contract error.
         *
         ...
         *   occurred yet.
         *
         *   An unsupported or invalid key causes the credential endpoint to respond with
         *   `invalid_proof`.
         *
         * example: Fetching attester public keys from the attester's hosted JWKS
         *
    ...
    formats: {
       ...
       *   to the token structure before signing. Customize the supplied `jwt.header` and
     ...
     *
     *   `true` binds the artifact to the current session; `false` leaves it usable after logout and
     *   marks the client authorization as persisting logout. The default returns `false` only when
     *   the source includes the `offline_access` scope.
     */
    expiresWithSession,
     ...
     *
     *   `true` issues a refresh token and `false` does not. The default requires both the
     *   `refresh_token` grant type and the `offline_access` scope.
     *
    ...
    pkce: {
       ...
       * pkce.required
       *
       * description: Configures if and when the authorization server requires clients to use
       * `PKCE`. This helper is called whenever an authorization request lacks the code_challenge
       * parameter. `false` allows the request to continue without PKCE, while `true` rejects it.
       */
     ...
     *
     *   The returned identifier MUST be a non-empty string that is stable for the same account and
     *   sector identifier while remaining unlinkable across sectors.
     *
     ...
     *   artifacts within the authorization server. Every static value and every synchronous
     *   callback result MUST be a positive safe integer number of seconds
     *   (`Number.isSafeInteger(value) && value > 0`). Zero, negative, fractional, `NaN`,
     *   infinite, and unsafe integer values are rejected. TypeScript represents this contract
     *   as `number`, so these constraints are enforced when the Provider is constructed for
     *   static values and whenever a configured callback is evaluated for dynamic values.
     *
    ...
    extraClientMetadata: {
       ...
       *   be synchronous; async validators and any returned thenable are rejected during
       *   runtime. To modify the current client metadata values (for the current key or any other)
       *   simply modify the passed in `metadata` argument within the validator function. `ctx` is
       *   provided for registration and update requests and is `undefined` for other Client
       *   construction paths.
       */
     ...
     *
     *   The current route and token models are available from `ctx.oidc`. `true` destroys the
     *   underlying Grant after its related token artifacts are revoked and emits `grant.revoked`;
     *   `false` preserves the Grant. The default preserves the Grant only when revoking an
     *   AccessToken at the revocation endpoint.
     */
    revokeGrantPolicy,
    ...
    interactions: {
       ...
       *   All checks belonging to a Prompt are evaluated concurrently. Checks within the same
       *   Prompt MUST NOT depend on evaluation order or on mutations performed by another check.
       *
     ...
     *   account identifier. The returned Account contains an `accountId` property and a
     *   `claims()` method that returns the claims supported by the issuer; `claims()` may also be
     *   asynchronous. Return `undefined` when the account cannot be loaded.
     */
    findAccount,
     ...
     *
     * The RefreshToken and Client are available as `ctx.oidc.entities.RefreshToken` and
     * `ctx.oidc.entities.Client`. `true` consumes the presented token and issues a rotated refresh
     * token; `false` continues without rotation. A configured literal Boolean applies that
     * decision without invoking a function.
     */
    rotateRefreshToken,
     ...
     *   server requirements.
     *
     *   `claims` and `header` are structured clones of the verified assertion Claims Set and JOSE
     *   Header, so mutating them does not alter the verified assertion. Throw an appropriate error
     *   to fail authentication. The default additionally requires an exact issuer identifier
     *   audience when the FAPI 2.0 profile applies.
     */
    assertJwtClientAuthClaimsAndHeader,
