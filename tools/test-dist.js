...
import {
  cpSync, mkdirSync, mkdtempSync, readdirSync, rmSync, symlinkSync, writeFileSync,
} from 'node:fs';
...
try {
  ...
  run('tar', ['-xzf', join(staging, tarball), '-C', staging]);

  const packed = join(staging, 'package');
  const contents = readdirSync(packed).sort();
  process.stdout.write(`\npacked ${tarball} containing ${contents.join(', ')}\n\n`);

  // the MIT notices for the code vendored into lib/ have to travel with it
  for (const required of ['lib', 'package.json', 'README.md', 'LICENSE.md', 'THIRD-PARTY-NOTICES.md']) {
  ...
  run('npm', ['install', '--install-strategy=nested', '--omit=dev', '--no-audit', '--no-fund', join(staging, tarball)], isolated);
