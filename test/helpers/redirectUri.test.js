...
describe('redirectUri helper', function() {
  it('does not modify the redirect_uri when it does not have path', function() {
    let result = redirectUri('http://client.example.com', {
      some: 'payload'
    });
  ...
  it('extends the query if part of the redirect_uri', function() {
    let result = redirectUri('http://client.example.com?other=stuff', {
      some: 'payload'
    });
  ...
  it('payload comes first', function() {
    let result = redirectUri('http://client.example.com?some=paylod', {
      some: 'other payload'
    });
  ...
  it('works with fragment and keeps query', function() {
    let result = redirectUri('http://client.example.com?present=query', {
      some: 'payload'
    }, 'fragment');
