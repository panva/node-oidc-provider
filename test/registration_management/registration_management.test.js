...
describe('OAuth 2.0 Dynamic Client Registration Management Protocol', function () {
  ...
  describe('Client Update Request', function () {
    ...
    it('responds w/ 200 JSON and nocache headers', setup({}, function (client) {
    ...
    it('allows for properties to be deleted', setup({ userinfo_signed_response_alg: 'RS256' }, function (client) {
    ...
    it('must contain all previous properties', setup({ userinfo_signed_response_alg: 'RS256' }, function (client) {
      // removing userinfo_signed_response_alg and having it defaulted
      // console.log(client);
      return agent.put(`/reg/${client.client_id}`)
      .set('Authorization', `Bearer ${client.registration_access_token}`)
    ...
    it('emits an event', setup({}, function (client) {
      const spy = sinon.spy();
    ...
    it('must not contain registration_access_token', setup({}, function (client) {
      // changing the redirect_uris;
      return agent.put(`/reg/${client.client_id}`)
      .set('Authorization', `Bearer ${client.registration_access_token}`)
      .send(updateProperties(client, {
    ...
    it('must not contain registration_client_uri', setup({}, function (client) {
      // changing the redirect_uris;
      return agent.put(`/reg/${client.client_id}`)
      .set('Authorization', `Bearer ${client.registration_access_token}`)
      .send(updateProperties(client, {
    ...
    it('must not contain client_secret_expires_at', setup({}, function (client) {
      // changing the redirect_uris;
      return agent.put(`/reg/${client.client_id}`)
      .set('Authorization', `Bearer ${client.registration_access_token}`)
      .send(updateProperties(client, {
    ...
    it('must not contain client_id_issued_at', setup({}, function (client) {
      // changing the redirect_uris;
      return agent.put(`/reg/${client.client_id}`)
      .set('Authorization', `Bearer ${client.registration_access_token}`)
      .send(updateProperties(client, {
  ...
  describe('Client Delete Request', function () {
    it('responds w/ empty 204 and nocache headers', setup({}, function (client) {
