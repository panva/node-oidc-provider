...
describe('requests without the openid scope', () => {
  ...
  describe('response_types and flows that work without scopes', () => {
    [undefined, 'api:read'].forEach((scope) => {
      describe(`when scope is ${scope ? `e.g. missing openid (${scope})` : 'omitted entirely'}`, () => {
        ...
        describe('response_type=token', () => {
          ...
          it('gets a token from the authorization endpoint', async function () {
            ...
            await this.wrap({ route: '/auth', verb: 'get', auth })
              ...
              .expect(auth.validateClientLocation)
              .expect(auth.validatePresence(['access_token', 'state', 'expires_in', 'token_type']));
